...
Models of $S^+$ are uniquely determined by the values of the
\emph{sources}, i.e., the start nodes of these linear paths.
...
If $\beta$ is not on the same path as $\alpha$ in $G_S$, define also
$\rho'(\beta') = \rho(\beta') + \rho(\gamma) - \rho(\beta)$ 
for any $\beta'$ on the same path as $\beta$ in $G_S$.
...
paths of $G_S$, so that $\rho'$ is a model of $S^+$. In each cases we
...
Use $\rho'(\beta)\geq\rho(\gamma)$ and $(\alpha\vee\beta)M \geq M(\beta)$
if $\beta$ is not on the same path as $\alpha$. 
