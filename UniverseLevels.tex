...
\begin{abstract}
We present rules for both externally and internally indexed sequences of universes \'a la Tarski. The internally indexed rules introduce a notion of level similar to Agda's  notion of level for universe polymorphism. However, our levels do not form a type, as they do in Agda. We first introduce rules for "ML"-style universe polymorphism, where judgments are indexed by level variables $\alpha_1, \ldots, \alpha_n$. Then we add rules for level-indexed product types $[\alpha]A$. Moreover, we present rules for cumulativity. We survey the historical evolution of universes in type theory and discuss the relationship between the level-indexed systems and the externally indexed systems as well as with Palmgren's super universe. 
\end{abstract}
...
Palmgren \cite{palmgren:phd} then proposed rules for an infinite external sequence of universes $\UU_n$ \`a la Tarski with decodings $\T_n$. In fact, Palmgren discussed two alternative versions of the rules for an external sequence of universes \`a la Tarski. 
...
Models of type theory can be expressed as categories with families (cwfs) with extra structure for modelling the rules for type formers.  A key feature of cwfs is that they can be expressed as models of a generalised algebraic theory in the sense of Cartmell \cite{cartmell}, and also that the extra structure for type formers can be expressed by adding more operations and equations to the generalised algebraic theory of cwfs. When we add new rules to type theory, it is desirable that the corresponding extra structure on  models also can be expressed in terms of operations and equations to the generalised algebraic theory of cwfs. We should therefore ask ourselves which the extra structure needed for modelling universe hierarchies.
