...
TODO:
\begin{enumerate}
\item The rules for $\NN^n$ and $\UU^n$ are notationally inconsistent (because the superscript means which universes $\NN^n$ \emph{lives in}, where it means which universe $\UU^n$ \emph{is}), as well as semantically irregular (because $\UU^n$ is in \emph{one} universe, whereas $\NN^n$ is in all universes).
...
\item If we place both of them in one universe, it should be a metatheorem all types have a unique universe, and otherwise all type should havve a minimal universe.
