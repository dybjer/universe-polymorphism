...
An \emph{undirected path} in $G_+$ is a sequence of nodes connected
...
all undirected paths in $G_+$ from $\delta$ to $\delta'$ have the same length.
...
If two nodes of $G_+$ are connected by an undirected path,
...
Given a node $\delta$ of $G_+$, all nodes at the same algebraic distance $k$ from 
...
Without loss of generality we may therefore assume that $G_+$ consists 
of a finite set of linear paths that are mutually disjoint and finite.

We can now prove a so-called \emph{small model property}.
\begin{lemma}\label{lem:small-model}
Let  $S$ be a finite set of constraints in reduced form such that all paths in $G_+$ are linear
