...
\begin{proof}
1. We first define by induction for any $l$ a variable $v(l)$ and a set $S(l)$ of 
...
First, $S(l)$ contains precisely as many symbols $+$ as $l$ 
(occurrences in names of fresh variables like in $v(l^+)$ do not count).
Secondly, every assignment of variables in $l$ can be extended to an
assignment validating $S(l)$.



By induction on the size of $l$ and $m$ in $l=m$. If both are variables, then we can
