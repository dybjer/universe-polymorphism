...
In intensional type theory we will not be able to justify all the laws for level expressions as definitional equalities. For example, we do not have $\max(m,n)=\max(m,n)$ definitionally.
...
We view Voevodsky's system as an extension of the system in \cref{sec:internal}
with judgments for {\em constraints}. A constraint is
an equation $l = m$, where $l$ and $m$ are level expression.
%Thus we have
%$$
...
Finally we have the judgment $\psi~(\Gamma)$ which expresses that the constraint set $\psi$
