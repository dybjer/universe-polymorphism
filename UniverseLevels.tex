...
Before we continue, we simplify $S_+$ without loss of generality.
Let $G_+$ be the directed graph with a node $\delta$ for every
variable $\delta$ occurring in $S$, and edges $\delta\to\epsilon$
for every constraint $\delta = \epsilon^+$ occurring in $S\+$.
Clearly, for $S\+$ to be consistent, for any to nodes $delta$ and $\delta'$,
all paths in $G_+$ from $\delta$ to $\delta'$ must have the same length.
This condition can be tested in quadradic time, and is also sufficient
for the consistency of $S_+$.

Given a node $\delta$ of $G_+$, all nodes at the same distance $k$ from 
$\delta$ must have the same value under any assignment. Without loss
of generality we may therefore assume that $G_+$ consists of a finite
set of linear paths that are mutually disjoint and finite.
