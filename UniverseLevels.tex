...
We proceed by elaborating the following three points.

\begin{enumerate}
\item (Transformation to reduced form.) 
...
Let $K_{S}$ be the number of occurrences of symbols $+$ in $S$.
If there is a model of $S$, then there is a model with values at most $K_{S}$.

\item (Stay above models.)
...
\begin{proof}
1. 
We first define by induction for any $l$ a variable $v(l)$ and a set $S(l)$ of 
constraints in reduced form such that $S(l)\models l = v(l)$.
...
and $S(l^+) = S(l) \cup \set{(v(l)\vee v(l))^+ =v(l^+)}$.
...
First, $S(l)$ contains precisely as many symbols $+$ as $l$ does,
...
Let $S$ be a set of constraints in reduced form.
...
If the constraint is $(\alpha\vee\beta)+b = \gamma$
...
a gap $k>0$, then we can lower the values of all variables with values above 
...
symbols $+$ in $S$. A simple translation to $0$ finally gives the desired small model.

3. Assume again that $S$ has been transformed to the reduced form as in point 1.
Let $M$ be a model of $S$ with values at most $K_S$ as in point 2. 
Let $\rho\geq M$ (pointwise)
and let $\rho'$ be as in point 3 above. In each of the three cases we
have to prove that $\rho'\geq M$ only for the variable that is lowered. 
...
In each step from $\rho$ to $\rho'$ in point three, exactly one variable is
lowered. This can be done at most $V K_S$ times, where $V$ is the number of variables.
This means that in at most $V K_S$ steps we find a model if there is one.
If $S$ has a model, then starting with $\rho$ constant $K_S$
and iterating the procedure in point 3 will find the unique maximal
model $M$ with values at most $K_S$. Each step requires a pass through all
...
\subsection*{Martin's example (Berry function)}
...
$$
The most general solution is $(A : \U_l) (B : \U_m)$ for $\U_l$ and $\U_m$ arbitrary.
...
If we write
$$
    (A : \U_l) (B : \U_m) (C : \U_n) \to A \times B \simeq C \times A \to B \times A \simeq C \times A,
$$
we need to solve the system of equations
$$
    \U_l \vee \U_n = \U_{l \vee m}
$$
$$
    \U_l \vee \U_n = \U_{l \vee m}
$$
...
$$
    (A : \U_l ) (B : \U_m ) (C : \U_{l \vee m} )
$$
$$
    (A : \U_l ) (B : \U_{l \vee m }) (C : \U_m )
$$
$$
    (A : \U_l \vee \U_m ) (B : \U_l) (C : \U_m )
$$
for $\U_l$ and $\U_m$ arbitrary.
