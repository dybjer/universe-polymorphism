...
We add $T_{l}^m(A) = A$ if $l = m$ THIS CANNOT BE SIMPLIFIED AS ABOVE?
...
In intensional type theory we will not be able to justify all the laws for level expressions as definitional equalities. For example, we do not have $\max(l,m)=\max(m,l)$.
