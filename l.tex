...
two base cases, 
(1) $S\prvH a\to a$ and 
(2) $S\prvH A\to b$ if $A\to b$ in $S$, and two induction cases,
\emph{weakening} (3) $S\prvH A,c\to b$ if $S\prvH A\to b$ and
\emph{cut} (4) $S\prvH A\to b$ if $S\prvH A\to c$ and $S\prvH A,c\to b$.
The following lemmas will be used later on.

\begin{lemma}\label{lem:plusbodyS}
Let $S$ be a set of clauses of the form $X\to y$ with $X\cap V = \emptyset$.
If $S\prvH A\to b$, then we have $b\in A$ or $S\prvH (A-V)\to b$
(without increasing the length of the derivation).
\end{lemma} 
\begin{proof} By induction on the definition of $S\prvH A\to b$.
The base cases and weakening are obvious. Regarding the cut,
assume $S\prvH A\to b$ is inferred from $S\prvH A\to c$ and 
$S\prvH A,c\to b$, and assume the lemma holds for the latter two (IH).
If $b\in A$ we are done. If $c\in A$ we are done by (IH). If neither of
the latter two is the case, we have $b=c$ or $S\prvH ((A,c)-V)\to b$ by (IH).
If $b=c$ we are done by (IH). If $S\prvH ((A,c)-V)\to b$ and $c\in V$ we are
done. If $c\notin V$, then we have $S\prvH (A-V),c\to b$
and $S\prvH (A-V)\to c$ and we get $S\prvH (A-V)\to b$ by a cut.
\end{proof}

For $X$ a set of atoms, define $X^+ := \set{x^+\mid x\in X}$.
For $S$ a set of clauses, define $S^+ := \set{X^+\to y^+ \mid (X\to y) \in S}$.

\begin{lemma}\label{lem:shift-up}
Let $S$ be a set of clauses, and $T=\set{v^+\to v \mid v\in V}$. 
Then we have:
$S\prvH A\to b$ if and only if $S^+ \cup T \prvH A^+\to b^+$.
\end{lemma} 
\begin{proof} From left to right is obvious.
The other direction is proved by induction on the length of the derivation of 
$S^+ \cup T \prvH A^+\to b^+$.
The base cases and weakening are obvious. 
Assume the lemma holds for shorter derivations (IH).
Regarding the cut, assume $S^+ \cup T \prvH A^+\to b^+$ is 
inferred from $S^+ \cup T \prvH A^+\to c$ and 
$S^+ \cup T \prvH A^+,c \to b^+$.
If $c=d^+$ for some $d$ we are done by an appeal to (IH) and cut. 
Otherwise we have $c\in V$ and proceed as follows.
The set of clauses $S^+ \cup T$ satisfies the condition of Lemma~\ref{lem:plusbodyS}.
Hence we have $b^+\in A^+,c$, so $b^+\in A^+$ and we are done, or
$S^+ \cup T \prvH ((A^+,c) -V) \to b^+$, that is, $S^+ \cup T \prvH A^+ \to b^+$
without increasing the length {\color{red}of the subderivation}. 
Now we are done by (IH).
\end{proof}
...
so that we get decidability of $S^\uparrow_C\prvH A\to b$ in
general. {\color{red} Use Lemma~\ref{lem:plusbodyS} and Lemma~\ref{lem:shift-up} TBD}.

Thus we get:

\begin{corollary}\label{cor:decidability}
For all $C,A,b,s,t$, $S^\uparrow_C\prvH A\to b$ and $C\prvL s=t$ are decidable.
\end{corollary}
...
We have $S^\uparrow_C\prvH V+k \to w+n+1$. However,
variables in $V-W$ cannot play a role above $n$.
{\color{red} TO BE ELABORATED}
Hence we must have $S^\uparrow_C\prvH W+n \to w+n+1$.
