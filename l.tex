...
\FYI{Since $S_C$ is finite, using Lemma~\ref{lem:f-sat},
...
\FYI{Proof: if $W=V$, then $g(x)>f(x)$ and we can infer $x+g(x)$ 
...
We would also like to thank Franz Baader for updating us on related
...
We thank both reviewers for their constructive comments and the link with
decidability results for description logics.
The title has been changed to better reflect the content of the paper.
The paper has been thoroughly reworked and a new section on complexity has been added.

For your convenience, changes related to the points below are in red.
The points (1)--(8) in one referee report are all done.
Below we give short comments to the points in the other review.
...
\item[Page 4, Thm. 3.1, induction step] Done.

\item[Page 4 and 5] Done.

\item[Page 4, line -4] No (!). We may need the whole set $W+n$ to infer each $w+n+1$.
Example: $x,y\to x+1$, $x,y\to y+1$.
