...
\section{Decidability}\ref{sec:decidability}
...
\section{Complexity analysis}\ref{sec:complexity}

All proofs in this note are constructive, so that they in fact
contain algorithms. In this section we shal argue
...
algorithms depending on $f$ is weakly polynomial. However, for the important
Corollary~\ref{cor:decidability} and \ref{cor:loopchecking}
the algorithms are strongly polynomial, that is, in PTIME.

The algorithms are basically just forward reasoning. 
However, since we have infinitely many Horn clauses, one has to take care
to terminate, which is explained in the inductive proofs. 
Moreover, termination should happen in a polynomial number of inferences,
each taking at most polynomial time.

In the following we list a complexity analysis of each of the results 
in Section~\ref{sec:decidability}. The correspondence between semilattices
and Horn clauses is well-known to be polynomial.

\begin{itemize}

\item The complexity of the test in Lemma~\ref{lem:f-sat}
is clearly polynomial in $|A|$ and $|f|$.

\item In Lemma~\ref{lem:secondary} one has to be careful:
even with only one clause $x,y \to y+1$ and $f(y)=0$ one needs
$f(x)$ inferences to arrive at the model $f(y)=f(x)+1$.
This is polynomial in the value of $f(x)$, but not in its
binary representation. 

However, the inequality (\ref{eq:f-ub}) gives an upper bound on 
values that can be generated, and $\max(f(V-W)) + \M <= |f| + \M$.
Clearly $\M < |S_C|$.
Inspection of the proof shows that the resulting $h$ can be
computed in time polynomial in $|f|$ and $|S_C|$.

\item The algorithm in the proof of Theorem~\ref{thm:main}
is driven by the induction on $|V|$. In each step of the induction
there is one application of Lemma~\ref{lem:secondary} ..
