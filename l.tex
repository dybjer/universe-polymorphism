...
The motivation for this problem comes from dependent type theory, where
the relevant operations on universe levels are taking the
supremum of two levels, and incrementing a level.
Ever since Skolem ??
we know that lattice theory is decidable, but this fact has
to be reconsidered in the presence of a successor function.
...
It is important that extensions
of the type checking procedure do not jeopardise decidability. 
The results of this note show that making typings depend
on a set of universe level constraints preserves the decidability
of type checking.

We depart from an algebraic definition of semilattices in which the
...
$x+k+1 \to x+k$ with $x\in V$ and $k\in \N$.
...
as well as all predecessor clauses. 
...
In particular we have the converse implication of the theorem.
\end{proof}

\section{Decidability}

In this section we prove the decidability of $S^+_C\prvH A\to b$.
By Theorem~\ref{the:LvsH} this is sufficient for
...
We end up with a subset $V'$ of $V$ and a (restricted)
$f : V' \to \N$.
...
- General case obtained most easily with $f: V\to -1,\Ninf$.
...
- Short discussion of alternative choices
  (0 level, total ordering, injective successor)
