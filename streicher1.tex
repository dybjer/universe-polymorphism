...
We have an operation which removes the annotations from $T_1$ to $T_2$.
$$
...
\begin{lemma}
  If $\Gamma\vdash t:A$ and $\Gamma\vdash u:A$ and $|t| = |u|$\footnote{syntactic identity; proof by induction on $\Gamma\vdash t:A$ and $\Gamma\vdash u:A$?} is $\beta$-normal then $\Gamma\vdash t = u:A$.
...
It is possible to present type theory with a cumulative hierarchy of universe as a generalised algebraic theory.
...
(This method was formalised in the work \cite{brunerie:initiality}.)
