...
There are at least three ways to solve this problem. In short:
one can extend type formation, one can refine how substitutions
act, and one can allow that contexts contain loops.
In the following we elaborate these three solutions.

\paragraph*{Extending type formation}

The description of the type theories in the paper is admittedly sketchy,
in particular the substitution of levels for level variables.
...
Add the following formation rule to the type theory in \cref{sec:constraints}:
$$
\frac{\Gamma\vdash A~\type~~~\Gamma\vdash l~\Level~~~\Gamma\vdash m~\Level}%
{\Gamma\vdash [l=m] A~\type}~~~\text{$\Gamma,l=m$ yields a loop}
$$
(NB $\Gamma,l=m$ is not a context.)
Types that are inferred by this rule can be considered to be empty
products in the context $\Gamma$. They are useless, but do no harm.
TODO or remove.

\paragraph*{Refining substitution}
...
Assume we have a substitution sub : H -> G  wih G and H contexts. Define:
...
Hello Marc,
Thanks for finding a way to get back to me. I’d been muling a bit over the issue, while nursing a bad cold the last two days.
  - The first thought I had was that I’d missed a simple fix of only checking for loop-freedom when checkin entailment of constraints (i.e., when there’s a « psi valid » premise),
 and otherwise allowing inconsistencies (non loop-free) contexts. That doesn’t work because one still loses subject reduction, because disabling a conversion rule in this way in a dependently typed system can break a type derivation.
 - I believe your fix can be made to work, but it would have to be more extensive, because one also has to worry about term substitution : for example, suppose we have [psi]T : Type where psi is consistent with the current context Gamma, and we now want to form F{[psi]T/alpha}; alpha might well occur in F in local contexts that are consistent with Gamma, but inconsistent with (Gamma, psi), so it is also necessary to tweak the definition of substitution to prune such cases, similar to what you are suggesting for universe level substitutions:
    ([phi]T){t/x} = [phi](T{t{phi}/x}), ([psi]T){phi} = unit if Gamma, phi, psi has a loop (assuming cumulativity).
    similar rules for <phi>u, and otherwise propagating substitution ans assumption specialisation.
Note that while we are at it we could also define ([psi]T){phi} = T when Gamma, phi entails psi, restrict the formation rule for [phi]T to contexts that do NOT entail phi (in addition to not being inconsistent with phi), with similar rules for <phi>t and level substitution do away with the conversion rules for [phi]T and <psi>t entirely.
  - While I think the semantics of this approach should work, I fear that equational and type checking reasoning may be awkward, forcing inconvenient case splitting on which constraints are entailed or inconsistent with the current context.
   - The approach I was mulling over in my fevered days was to simply embrace the inconsistencies rather than hunt them down, that is, simply make the logic inconsistent in an inconsistent context by
adding a rule
   Gamma has a loop |- t = Omega : T (Note that this entails Omega : Omega)
    Then the pruning you suggested can be performed eagerly while type checking by replacing everything with Omega when the context becomes inconsistent, so the semantics can still be restricted to consistent contexts, but as pruning is localised to the type checking algorithm it wlll not interfere as much with manipulating and reasoning about terms…
   Cheers,
Georges
