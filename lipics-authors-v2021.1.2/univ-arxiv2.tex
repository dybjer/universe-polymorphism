...
and then elaborate some solutions.

As an example, assume one has inferred a typing $\alpha,\beta \vdash 
\lam{\alpha<\beta} t : [\alpha<\beta] T$.
Substituting $\alpha$ for $\beta$ gives the typing $\alpha \vdash 
\lam{\alpha<\alpha} t : [\alpha<\alpha] T$.
However, the typing $\alpha \vdash [\alpha<\alpha] T~\type$
cannot be inferred with the formation rule for constraint-indexed
products (the first rule in \cref{fig:restriction}).
The context extension rule only allows for extension with constraints 
provided that the resulting context is loop-free. 
Thus $\alpha<\alpha$ can never occur in a context.

As a consequence of the above example the type theory in \cref{sec:constraints}
fails to have some usual, and very desirable, metatheoretic properties.
There are several ways to solve this problem.
In the following we give two solutions.

\paragraph*{Allowing loops in contexts}
\def\gg{{\circlearrowleft}}% loop 

This solution has been suggested by Georges Gonthier, for which
we are very grateful. The idea is to allow loops in contexts,
making sure that the type theory collapses in contexts that
contain a loop. 
...
Recall that a level expression $l$ is a \emph{loop}
...
Now add a new symbol $\gg$ to the type theory, plus the following rules.

\begin{figure}[h!]
$$
\frac{l<l~\valid}{\gg~\type}~~~~~~~~
\frac{A~\type~~~l<l~\valid}{\gg = A}~~~~~~~~
\frac{t:A~~~l<l~\valid}{\gg = A}~~~~~~~~
\frac{t:A~~~l<l~\valid}{\gg = t}
\belowdisplayshortskip 1pt
$$
 \caption{Rules for collapsing}%
  \label{fig:restriction}
\end{figure}
These rules clearly collapse the type theory (is the 3rd rule needed?).

\paragraph*{Making substitutions explicit}

For this solution we keep the contexts as in the paper, loop-free.
The description of the type theories in the paper is admittedly sketchy,
in particular regarding the substitution of levels for level variables.
...
sense of \cite{bezem-coquand:lattices}. 
We should have stipulated in \cref{sec:constraints} that level substitutions
are lattice homomorphisms between these contexts. Thus we cannot
substitute $\alpha$ for $\beta$  in the context $\alpha,\beta,\alpha<\beta$
since $\alpha,\alpha<\alpha$ is not a context. However, we still
have the problem exemplified in introduction to this appendix.
