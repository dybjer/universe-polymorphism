...
We refrain from elaborating the above solution in detail here, as it will complicate the syntax significantly. Instead we will present
...
First we liberalize the context extension operation $\Gamma,\psi$ in 
\cref{sec:constraints} by dropping the second condition, 
that the extended context $\Gamma,\psi$ is loop-free. 
...
The idea of adding the following rules is to collapse the type
...
(The lattice presented by the level
variables and constraints in $\Gamma$ can still be non-trivial even
if $\Gamma$ yields a loop.)
In this solution substitutions no explicit substitutions are needed.
