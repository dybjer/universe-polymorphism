...
$$
(NB $\Gamma,l=m$ is \emph{not} a context here.)
...
\begin{verbatim}
Hello Marc,
Thanks for finding a way to get back to me. I’d been muling a bit over the issue,
 while nursing a bad cold the last two days.

  - The first thought I had was that I’d missed a simple fix of only checking for
   loop-freedom when checkin entailment of constraints 
   (i.e., when there’s a « psi valid » premise),
 and otherwise allowing inconsistencies (non loop-free) contexts. 
 That doesn’t work because one still loses subject reduction, 
 because disabling a conversion rule in this way in a dependently 
 typed system can break a type derivation.
...
with the conversion rules for [phi]T and <psi>t entirely.
  ...
  - While I think the semantics of this approach should work, I fear 
  that equational and type checking reasoning may be awkward, 
  forcing inconvenient case splitting on which constraints are entailed or 
  inconsistent with the current context.
   ...
   adding a rule Gamma has a loop |- t = Omega : T 
   (Note that this entails Omega : Omega)
    ...
    checking by replacing everything with Omega when the context becomes
     inconsistent, so the semantics can still be restricted to consistent
      ...
      it wlll not interfere as much with manipulating and reasoning about terms…
   Cheers,
Georges
\end{verbatim}
\end{document}
