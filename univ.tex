...
\begin{abstract}
  ...
  The goal of this paper is to refine and extend Voevodsky's proposal for a type
  theory with universe polymorphism \cite{VV}, in which judgments
  ...
  universes \`a la Tarski. We add judgments $l\ \Level$ for internal
  ...
  $l \vee m$.  We extend this theory with rules for level-indexed
  product types.  Finally, and this is the main contribution of
  this paper, we add constraints and constraint-indexed product types,
  and compare the resulting type theory to Voevodsky's.
...
This included the expectation that it might be feasible for a user of 
type theory to explicitly assign universe levels to their types, 
simply adding updated versions of earlier
...
since universe level inference can be a costly operation, 
slowing down type-checking significantly. 
Moreover, "universe inconsistencies" can be hard to explain to the user.

 In order to cope with these issues, J. Courant \cite{Courant02}
suggested to have explicit level universes, 
with a sup operation (see also \cite{herbelin05}).
...
hence $\UA_n$ is in $\UU_{n+1}$.
If we have a type of levels, as in Agda \cite{agda-manual} 
or Lean \cite{moura:lean},
...
In this work {\color{red} EDIT ME} we propose a system that refines
Courant's \cite{Courant02} and Voevodsky's \cite{VV} systems as follows:
  \begin{enumerate}
  \item
    We generalize levels from the natural numbers to elements of an arbitrary sup-semilattice with successor.
...
We then explain how to add an externally indexed sequence of universes 
...
We then further extend the theory with level-indexed products of 
...
Section~\ref{sec:internal} with constraints between level variables
and constraint-indexed products of types $[\psi]A$ and corresponding
abstractions $\lam{\psi}A$. This goes beyond \cite{VV} where the
constraint set is fixed (but arbitrary) on the level of the type theory.
We consider this extension the main contribution of our paper.

Finally, we extensively compare our type theory with Voevodsky's
...
see \cite{bezem-coquand:lattices}.%
\footnote{
...
quotienting $\Lev_\Gamma$ in
Figure~\ref{fig:leveleq} from Section~\ref{sec:internal}.
...
In the latter case, $\Lev_{\alpha \Level}$ is isomorphic to the 
natural numbers with successor and $\max$. Still, we get some more
...
{\color{red} has been replaced by relevant references
to \cite{bezem-coquand:lattices} at various places.}
...
and Voevodsky's in the following list.
 \begin{enumerate}
\item In \cite{VV}, u-levels are natural numbers, even though u-level
...
operation ({\color{red} inflationary endo?}). 

\item In \cite{VV}, constraints are introduced, once and for all,
at the level of the theory. In our proposal they are introduced
at the level of contexts 

\item There are no level-indexed products or abstractions in \cite{VV},
although rules 7 and 10 in \cite[Sec. 3.4]{VV} bear some resemblance 
to the 3rd and 4th rule in Figure~\ref{fig:restriction}.

\item There are no constraint instantiation rules in \cite{VV}
such as the 2nd and the 5th rule in Figure~\ref{fig:levindprod}.

\item Rules 29 and 30 in \cite[Sec. 3.4]{VV} postulate
cumulativity with explicit coercion. In our proposal cumulativity
is optional, see Appendix 1.

\item The type theory in \cite{VV} is Tarski-style.
