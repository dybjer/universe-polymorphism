...
We  have a new context extension $\Gamma,\psi$ if $\psi$ is a consistent
finite set of constraints in $\Gamma$. A finite set of constraints is 
{\em consistent} if it does not create a {\em loop},
i.e., a level expression $l$ such  that $l<l$ modulo this list of constraints.
...
MB: We seem to need some rule like this:
\begin{figure}[H]
  \caption{Rule for level equality depending on constraints}%
  \label{fig:depleveleq}
$$
\frac{\Gamma\vdash l~\Level~~~~~\Gamma\vdash m~\Level}
...
$$
\end{figure}
Here, $\Lev/\Gamma$ is the sup-semilattice with $\_^+$
with generators (level variables) in $\Gamma$ modulo the constaints in $\Gamma$, 
...
{\color{red} Questions. 
   \begin{enumerate}
   \item Why is $(\psi)$ in parenthesis? I would say it is implied by
   the context, not part of. Replace $(\psi)$ by 
   $\Lev/\Gamma \models \psi$ as explained above?
   With the rule in Fig.\ref{fig:depleveleq} we could write $\psi$
   just as a premiss, without the parenthesis.
   \item Could one view $[\psi]A = A$ and $\lam{\psi}{t} = t$ as
   the elimination rules? 
   \item Replace $\psi = 1$ by $\Lev/\Gamma \models \psi$? Or just
   write $\psi$ as a premiss?
   \end{enumerate}}
...
\item Voevodsky allows inconsistent constraints. 
