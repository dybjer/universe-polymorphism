...
$$
\frac{}{()\vdash}~~~~~~~\frac{\Gamma\vdash A~\type}{\Gamma,x:A\vdash}~~~~~~
...
for types and terms: $\mylam{x}{A}{b}$, $\app{c}{a}$,
$(a,b).2$, and so on. More are to come. Conversion rules for
$\Pi$ and $\Sigma$ been given in \cref{fig:convPiSig},
and those rules imply that $=$ is an equivalence.
We want that $=$ is a congruence for all syntactic constructs.%
\footnote{Congruence for $\mylam{x}{A}{b}$ and $(a,b)$ is
implied by the rules in \cref{fig:convPiSig} in a clever way.}
Instead of giving tedious congruence rules for 
each addition to the syntax, we formulate the following schema.
Here $\const(t_1,\ldots,t_n)$ stands for a syntactic construct
$\const$ parametrized by $t_1,\ldots,t_n$.

\begin{figure}[h!]
  \caption{General schema for congruence rules}\label{fig:congr}
$$
\frac{t_1 = t'_1 : T_1 ~~\ldots~~ t_n = t'_n : T_n}
{\const(t_1,\ldots,t_n) = \const(t'_1,\ldots,t'_n) : T}
$$
\end{figure}

We now introduce the type of natural numbers $\NN$ as an example of an
inductive data type, with the eliminator $\NRapp{a}{g}$ as an example of a 
parametrized syntactic construct.
...
We also have (based) identity types $\Idapp{A}{a_0}{a_1}$ for $A~\type$, 
...
$$
\frac{a_0:A ~~~~ a_1:A}{\Idapp{A}{a_0}{a_1}~\type}~~~~~~~
...
We present a version without cumulativity. The version with cumulativity is presented in the Appendix.
$$
\UU_n~\type~~~~~~
...
and a code for the previous universes, for $n>m$
$${\UU^{n}_{m}}:\UU_{n}$$
$$\T_{n}({\UU^{n}_{m}}) = \UU_{m}
