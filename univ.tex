...
We will now present a complete\footnote{We recall that congurence rules are implicit.}
...
and a code for the previous universe, for $l<n$
$${\UU_l^{n}}:\UU_{n}$$
$$\T_{n}({\UU_l^{n}}) = \UU_{l}$$
...
{\Id~A~a_0~a_1:\UU_n}
$$
and, for $l<n$, we add ${\UU_l}:\UU_{n}$.

\medskip

For the version with cumulativity, we add the rules, for $l<n$
$$
\frac{A:\UU_{l}}{A:\UU_{n}}
$$
and the rules for products and sums can be simplified to
$$
\frac{A:\UU_{l}~~~~~~B:\UU_l(x:A)}
     {\mypi{x}{A}{B}:\UU_{l}}~~~~~~~~~
\frac{A:\UU_{l}~~~~~~B:\UU_l(x:A)}
     {\mysig{x}{A}{B}:\UU_{l}}~~~~~~~~~
$$
...
We then have a new context extension $\Gamma,\psi$ if $\psi$ is a {\em consistent}
...
$$
\UU^{\alpha}:\UU_{\beta}~(\alpha~\beta~\Level,\alpha<\beta)
$$
...
$$
t:    \Pi_{A:\UU_\alpha~{B}:{\UU_\beta}~{C}:{\UU_\gamma}}
    {~~\Id\,\UU_{\alpha \vee \beta}\, (A\times B)\,(C \times A)
    \to \Id\,\UU_{\alpha \vee \gamma}\, (B\times A)\,(C \times A)}
$$
    under the constraint  $\psi$ which is $\alpha \vee \beta = \alpha \vee \gamma$.
...
$$
    ...
    It is now possible to instantiate this general operation to levels and use it
    $u~l~m~n$ provided the levels $l,m,n$ satisfy the constraint $\psi$.

    We have thus complemented Voevodsky's proposal \cite{VV}: we can not only declare and define
    constants given universe levels satisfying some constraints, but we also provide a mechanism
...
in the Calculus of Constructions extended with a predicative system of universe \cite{coq86}.
