...
\footnote{PD: One may argue that Courant already had universe levels  and universe level equations as \emph{judgments}
...
We also have level equality judgments $\Gamma\vdash l = m~\true$ 
and want to enforce that judgments are invariant under level equality. 
To this end we add the rule that $\Gamma\vdash l = m~\true$
...
judgments are invariant under level equality: if $l=m~\true$ and
${a(l/\alpha) : A(l/\alpha)}$, then ${a(m/\alpha) : A(m/\alpha)}$.

We will now add rules for internally indexed universes.
Note that $l<m~\true$ is shorthand for the level equality judgment 
$m= l^+ \vee m ~ \true$.
\begin{figure}[H]
...
\frac{l<m~\true}{\UU^{m}_{l}:\UU_{m}~~~~~~\T_{m}({\UU^{m}_{l}}) = \UU_{l}}
\belowdisplayshortskip 0pt
$$
\end{figure}
...
through inversion lemmas.\footnote{Except the rules without assumptions, such as $\NN^{l}:\UU_{l}$.}
...
we can derive $A = B : \UU_l$ from $\T_l(A) = \T_l(B)$. {\bf Conjecture?}
 ...
 \footnote{Maybe we should write some more about this. It may not be clear to the reader what it means for a system to be "generalized algebraic"?}
...
that $\UU^m_l:\UU_m$ if $l<m~\true$, that is, if 
...
We also have a new judgment form $\Gamma\vdash\psi~\true$ that expresses 
that the constraints in $\psi$ hold in $\Gamma$, that is.
are implied by the constraints in $\Gamma$. If there are
no constraints in $\Gamma$, this amounts to the same as in
Section~\ref{sec:internal}. Otherwise it means that the constraints
in $\psi$ hold in the sup-semilattice with $\_^+$ presented by $\Gamma$.
As shown in \cite{bezem-coquand:lattices}, 
this condition, as well as loop-checking, is decidable in polynomial time.

%\begin{figure}[H]
%  \caption{Rules for constraints}\label{fig:constraints}
%$$
%\frac{\Gamma\vdash l~\Level~~~~~\Gamma\vdash m~\Level}
%     {\Gamma\vdash l=m~\true}~~\Lev_\Gamma \models l=m~~~~~~
%\frac{\Gamma\vdash l=m~\true~~~~~\Gamma\vdash \psi}
%     {\Gamma\vdash\set{l=m}\cup\psi}
%\belowdisplayshortskip 0pt
%$$
%\end{figure}
...
\frac{\psi~\true}{[\psi]A = A}~~~~~~~~~
\frac{\psi~\true}{\lam{\psi}{t} = t}
\belowdisplayshortskip 1pt
$$
\end{figure}
