...
<<<<<<< HEAD
It should be noted that the current version of Coq also supports explicit
universes, see \cite{coq:univpoly}.
%One difference with \cite{SozeauTabareau:coq} is that our treatment of universe polymorphism is explicit.
=======
%One difference with \cite{SozeauTabareau:coq} is that our treatment of 
%universe polymorphism is explicit. %Coq’s support ranges from fully explicit to entirely implicit universe and constraint annotations.
>>>>>>> dc5b957bb13a38ab6086395505ee6e706643adc3
...
extensionality. (Sozeau has explained to us that one can define
global univalence in a module in Coq and get some of the expressivity
of level-indexed products through Coq's module abstraction mechanism.)
...
Without being able to declare explicitely such constraints, % are not available in Agda and Lean
...
%We will make further remarks on the relationship between our work and the latter below.
...
%% {\color{red}One goal of \cite{SozeauTabareau:coq} is to extend Coq's system of (implicit) \emph{global} universes with \emph{global} constraints with support for (explicit) \emph{local} products of universes and contraints in definitions. Such definitions can be reused with any instantiation whose local constraints are implied by the global constraints. Such generic definitions may depend on previous generic definitions, but are otherwise in an empty context. The latter is one difference with our explicit polymorphism provided by level- and constraint-indexed products in any well-typed context.}
...
%\item
%% We could have similar rules for adding constants, where we would use our level-abstraction when defining universe-polymorphic constants and our level-application when instantiating them.
