...
As in \cite{Cardelli87}, we can distinguish between {\em implicit} and {\em explicit}
polymorphism\footnote{``Parametric polymorphism is explicit when parametrization is obtained by
...
types can contain type variables which are unknown, yet to be determined, types'' \cite{Cardelli87}.}. 
One difference with the treatment in \cite{SozeauTabareau:coq} is that our treatment of universe polymorphism
is explicit.
...
operation. {\color{red}In the abstract setting, for example,
