...
We later learned about the closely related work by Sozeau and Tabareau \cite{SozeauTabareau:coq}. As already mentioned the most important difference is between their ML-like polymorphim and the explicit polymorphism provided by our level- and constraint-indexed products. Moreover, our constraint languages differ: their constraints are equalities or (strict) inequalities between level variables, while ours are equalities between level expressions.

Apart from this, they make several presentation choices different from ours.
%\begin{itemize}
%\item 
For example, their systems are \`a la Russell and based on PTS with subtyping. Universes are called $\Type_i$ with $\Type_i : \Type_{i+1}$. Our universes are \`a la Tarski and called $\UU_m$ with codes $\UU^m_l : \UU_l $ for $l < m$.
%\item
Moreover, they have special rules for introducing universe-monomorphic and universe-polymorphic constants, and a rule for instantiating the latter. We could add similar rules for adding constants, where we use our notion of level-abstraction when defining universe-polymorphic constants.
%\item
...
%
%\item
Finally, we should mention that they present a system with an impredicative universe and only $\beta$-conversion, while we present a predicative system with $\eta$-conversion as well.
%\end{itemize}
