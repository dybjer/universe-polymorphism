...
\newcommand{\Rfapp}[1]{\sapp{\refl}{#1}}
\newcommand{\Japp}[3]{\sapp{\JJ}{#1,#2,#3}}
...
inductive data type, with the eliminator $\NRapp{P}{a}{g}{n}$. 
...
$$
$$
\frac{\NRapp{P}{a}{g}{n}:P(n/x)}{\NRapp{P}{a}{g}{\ZERO} = a: P(\ZERO/x)}
~~~~~~~
\frac{\NRapp{P}{a}{g}{n}:P(n/x)~~~~~n:\NN}
{\NRapp{P}{a}{g}{\Sapp{n}} = g~n~\NRapp{P}{a}{g}{n}: P(\Sapp{n}/x) } 
$$
\end{figure}
...
$\Japp{C}{a}{d}$, all examples of parametrized syntactic constructs
...
{\Japp{C}{a}{d}:\mypi{x}{A}{\mypi{p}{\Idapp{A}{a}{x}}}{C}~~~~~~~~
 \Japp{C}{a}{d}~a~\Rfapp{a} = d : C(a/x,\Rfapp{a}/p)}
$$
\end{figure}
...
\end{figure}
%
NB\footnote{$\NN^{0}:\UU_{n}$ would spoil uniqueness of types.
However, why not $\NN^{n}:\UU_{n}$ and $\T_{n}(\NN^{n}) = \NN$?}
\begin{figure}[H]
...
for \emph{level variables}, and $l,m,\dots$ for \emph{level expressions} which are built from level variables
...
~~~~~ \frac{l=m ~~~~~ a:A~(\alpha~\Level)}{a(l/\alpha) = a(m/\alpha)}
\belowdisplayshortskip 0pt
$$
\end{figure}
...
We can  now add rules for internally indexed universes:
\begin{figure}[H]
...
\begin{remark}
Old material that has possibly become obsolete (see previous para).
$$
...
{\bf Here is a minor problem: we only have externally} 
$\NN^{0}:\UU_{0}$.
...
{\bf The following is not duplicated from the rules for externally indexed 
universes with external numbers and is placed in \cref{fig:typeUl} 
to stress the fact
that the side-condition $n<l$ is upgraded to a level judgment.}
