...
is very elegant and is the basis of several
proof assistants. It however has some unnatural limitation: it is not possible in this system to talk
about an arbitrary type, or about an arbitrary structure, and it is not possible to formulate the collection
of e.g. all groups, as needed in category theory. In order to address these limitations, Martin-L\"of
\cite{ML71,ML71a} introduces a system with a type $V$ of all types. A function $A\rightarrow V$ in this system can
...
for this example avoid to have to declare a constraint by writing systematically $\gamma\vee\alpha^+$.
...
$$
   ...
   We can instantiate this constant $c$ on two levels $l$ and $m$, and this will be of type
