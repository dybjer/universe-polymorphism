...
Courant's \cite{Courant02} and Voevodsky's \cite{VV} systems as follows:
  \begin{enumerate}
  \item
    We generalize levels from the natural numbers to an arbitrary join semilattice with successor.
...
$$
can be written (note that we have left out $A~\type$ in the first rule)
$$
%\frac{\Gamma,x:A\vdash B}{\Gamma\vdash \Pi (x:A) B}~~~~~~~~~
...
The rules for $\Pi$ are the ones we used as example in the previous section.
For $\Sigma$ we have:
$$
...
The conversion rules are presented in Figure \ref{conv-basic}.

\begin{figure}
  \caption{Conversion rules for basic type theory}\label{conv-basic}
$$
...
%%   \footnote{PD: We omit some other general rules for equality reasoning, but since we say "The main conversion rules" we don't need to specify all rules. }
%% $$
%% \frac{ a:A~~~~~~ A~ \conv~ B}{ a:B}~~~~~~~~~
%% \frac{ a ~\conv~a':A~~~~~~ A  ~\conv~ B}{ a ~\conv~a':B}
%% $$
...
$$
\frac{A~\type~(\psi)}{[\psi]A~\type}~~~~~~~~~~~~~~~~~\frac{\lam{\psi}{t}:A~(\psi)}{t:[\psi]A}
$$
...
$l_1~\dots~l_m$ satisfy the constraint $\psi$.
 ...
 We can summarize as follows the differences between the present systems and Voevodsky's:
 \begin{itemize}
\item Voevodsky's levels are natural numbers with $0,l^+, l\vee m$.
