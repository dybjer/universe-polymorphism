...
{\color{red}
\paragraph{Main contributions.} Like Courant we present a type theory with 
universe levels  and universe level equations as \emph{judgments}. However, we have no level 0, and our universe levels only form a sup-semilattice with an inflationary endomorphism. Moreover, we add 
...
Moreover, we present a type theory with constraints as \emph{judgments} extending Voevodsky's system. We also add \emph{constraint-indexed products} to this system.
}%endcolor
%\footnote{PD: One may argue that Courant already had universe levels  and universe level equations as judgments, so this is not really a novelty in our paper. }
...
$l=m$ in the free sup-semilattice with $\_^+$ and generators 
...
by internal levels. (To rules without assumptions, 
such as in Fig.~\ref{fig:NIdU}, we need to add assumptions like $n~\Level$,
...
we can derive $A = B : \UU_l$ from $\T_l(A) = \T_l(B)$. {\bf Conjecture?\footnote{PD: how is this proved?}}
 ...
 obviously\footnote{PD: if the rule is admissible, then the system is generalized algebraic anyway.} generalized algebraic anymore, since the embedding rule gives rise to a conditional equation. (A system is generalized algebraic if it can be characterized as an initial model of a generalized algebraic theory in Cartmell's sense \cite{cartmell:apal}. See also the paper \cite{bezem:hofmann} for more information about generalized algebraic theories for dependent type theories and their initial models.)
...
Note that $\alpha<\beta$ holds iff 
$\beta$ is of the form $\gamma\vee\alpha^+$. \footnote{PD: $\alpha$ and $\beta$ are level variables, not level expressions. So $\beta$ cannot be of the form $\gamma\vee\alpha^+$.}
...
However, Voevodsky does not describe a mechanism to {\em eliminate}
universe levels and constraints, and this is what we present next.\footnote{What about saying precisely what this means?}
...
sup-semilattice with $\_^+$ presented by $\Gamma$. {\color{red}We add the rule
that $\Gamma\vdash l=m$ when $\Gamma\vdash\set{l=m}~\valid$.\footnote{PD: isn't this superfluous, since it follows from the first sentence of the paragraph?}}
...
at the level of contexts.\footnote{PD: is this a substantial difference, if we consider the theory we get if we remove constraint-indexed products?}
%
There are no level-indexed products or abstractions in \cite{VV}.
%
\footnote{PD: I would remove "There are no constraint instantiation rules in \cite{VV}
such as the 2nd and the 5th rule in Figure~\ref{fig:levindprod}" since these are among the rules for level-indexed products}.
%
