...
We later learned about the closely related work by Sozeau and Tabareau \cite{SozeauTabareau:coq}. We have already explained the most important differences, and will now list some of the presentation choices that obscure a comparison between their systems and ours.

%\begin{itemize}
%\item 
Their systems are \`a la Russell and based on PTS with subtyping. Universes are called $Type_i$ with $Type_i : Type_{i+1}$. Our universes are \`a la Tarski and called $U_m$ with codes $U^m_l : U_l $ for $l < m$.

%\item
They have special rules for introducing monomorphic and universe-polymorphic constants, and a rule for instantiating the latter. We could add similar rules for adding constants, where we use our notion of level-abstraction when defining universe-polymorphic constants.
