...
universe levels  and universe level equations as \emph{judgments}. Moreover, we don't restrict the levels to be natural numbers. Instead we just assume that they form a sup-semilattice with an inflationary endomorphism. In this way all levels are built up from level variables by a successor operation and a binary supremum operation. Unlike most other systems, we do not have a level constant zero for the first universe level. As a consequence all types involving universes depend on level variables; they are {\em universe polymorphic}.
...
Finally, we present a type theory with constraints as \emph{judgments} similar to the ones by Sozeau and Tabareau \cite{SozeauTabareau:coq} and Voevodsky \cite{VV}  and extend this type theory with \emph{constraint-indexed products}.
...
Since an assumption of global function extensionality can replace many assumptions of the local function extensionality (provided by the ML-polymorphism), this can also give rise to shorter code, see for example
%https://
\begin{verbatim}
...
We later learned about the closely related work by Sozeau and Tabareau \cite{SozeauTabareau:coq}. As already mentioned the most important difference is between their ML-like polymorphim and the explicit polymorphism provided by our level- and constraint-indexed products. Apart from this, there are several presentation choices that obscure a comparison between their systems and ours.

%\begin{itemize}
%\item 
Their systems are \`a la Russell and based on PTS with subtyping. Universes are called $\Type_i$ with $\Type_i : \Type_{i+1}$. Our universes are \`a la Tarski and called $\UU_m$ with codes $\UU^m_l : \UU_l $ for $l < m$.

%\item
They have special rules for introducing universe-monomorphic and universe-polymorphic constants, and a rule for instantiating the latter. We could add similar rules for adding constants, where we use our notion of level-abstraction when defining universe-polymorphic constants.

%\item
Their constraints are only between level variables, while our constraints are between level expressions.
...
%\item
Finally, a difference, which is orthogonal to universe polymorphism, is that they present a system with an impredicative universe and only $\beta$-conversion, while we present a predicative system with $\eta$-conversion as well.
%\end{itemize}
...
The authors are grateful to the anonymous referees for feedback, among other things on the relationship between our work and Sozeau and Tabareau's.
