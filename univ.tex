...
one would instead need to write four separate definitions.
 ...
 Suprisingly, if we add a least level $0$ to the term levels (like in Agda) then there {\em is} a most general solution,
 namely $l = \alpha\vee\beta\vee\delta,~m = \beta\vee\gamma,~n = \alpha\vee\gamma$\footnote{We learnt this from
 Th. Felicissimo, with a reference to the work \cite{FBB:impred2pred}.}, since it can be seen as an instance of
