...
\begin{lemma}
 % If $\Gamma\vdash t:A$ and $\Gamma\vdash u:A$ then $\Gamma\vdash t = u:A$.
  ...
  \begin{itemize}
    ...
    \item or $\Gamma\vdash A = \UU_k$ and $\Gamma\vdash B = \UU_l$ and $\Gamma\vdash \sT_k^m(t) = \sT_l^m(u):\UU_m$ with $m = \max(k,l)$.\footnote{TC:  Lemma 4.0.1 is not valid, even with normalisation. A counter-example is   $\lambda(N, U_{k+1}, U^k_n) $  and $\lambda(N, U_{l+1}, U^l_n)$    in T1
 Both correspond to $\lambda(N, U_n)$ in T2.

 Hopefully, we can still have that if  $G \vdash A$ and  $G \vdash B$ and $|A| = |B|$ then $G \vdash A = B$ and we need a refinement of Lemma 4.0.1 and we probably need to consider beta normal form.
 
 PD: But if the translation from T2 to T1 is typed, that is, we have 
 $[[ a ]]_{\Gamma,A} \in \Tm_{T1}([[ \Gamma ]], [[ A ]]_\Gamma)$ for $a \in \Tm_{T2}(\Gamma,A)$, then this is not a problem, is it?}
  \end{itemize}
\end{lemma}
