...
A model of a gat $\Sigma$ is an object in the category $\CwF_\Sigma$ of cwf with extra structure for the sort symbols and operator symbols in $\Sigma$ satisfying the equations \cite{bezem:hofmann}. For example, in the term model cwf of the gat for cwfs $\Sigma^\CwF$ we have $\ctx \in \Ty(1), \sub \in \Ty(1.\ctx.\ctx[\p])$ and use $\sub(\Delta,\Gamma)$ as shorthand for 
$\sub[\tuple{\Delta,\Gamma}]$, where $\tuple{a,b}$ abbreviates $\tuple{\tuple{a}, b}$, $\tuple{a}$ abbreviates $\tuple{\tuple{}, a}$, etc. Operator symbols are interpreted as terms. For example, $\id \in \Tm(1.\ctx,\sub(\q,\q))$ and $\id_\Gamma$ is shorthand for $\id[\tuple{\Gamma}]$. We thus get a cwf with an internal cwf, see \cite{bezem:hofmann} for more information.
