...
A model of a gat $\Sigma$ is an object in the category $\CwF_\Sigma$ of cwfs with extra structure for the sort symbols and operator symbols in $\Sigma$ satisfying the equations \cite{bezem:hofmann}. For example, in the term model cwf of the gat for cwfs $\Sigma^\CwF$ we have $\ctx \in \Ty(1), \sub \in \Ty(1.\ctx.\ctx[\p])$. Operator symbols are interpreted as terms. For example, $\id \in \Tm(1.\ctx,\sub(\q,\q))$ where $\sub(\Delta,\Gamma)$ is shorthand for 
