...
\paragraph{Interpretation in the term model.} Sort symbols are interpreted as types in the term model cwf, see \cite{bezem:hofmann}. We have $\ctx \in \Ty(1), \sub \in \Ty(1.\ctx.\ctx[\p])$ and use $\sub(\Delta,\Gamma)$ as shorthand for 
$\sub[\tuple{\Delta,\Gamma}]$, where $\tuple{a,b}$ abbreviates $\tuple{\tuple{a}, b}$, $\tuple{a}$ abbreviates $\tuple{\tuple{}, a}$, etc.

Operator symbols are interpreted as terms in the term model. For example, $\id \in \Tm(1.\ctx,\sub(\q,\q))$ and $\id_\Gamma$ is shorthand for $\id[\tuple{\Gamma}]$. 
