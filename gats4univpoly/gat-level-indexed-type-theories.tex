...
Moreover, the gat of cwfs has 10 operator symbols for the basic operations of cwfs (identity and composition of context morphisms, substitution in types and terms, empty context, context extension, empty context morphism, context morphism extension, projection morphism, last variable term). To illustrate the notation we show the typings of identity and composition of context morphisms:
\begin{eqnarray*}
...
In the gat for externally indexed universes we have infinitely many operator symbols:
\begin{eqnarray*}
...
\end{eqnarray*}
one for each $l, l' , m \in \NN$ with $l < m$. Note that codes for $\Pi$ are doubly indexed and $l \vee l' = \max(l,l')$.

Again, we have left some arguments to operator symbols implicit. For example, the decoding operators $\Ta_l$ are binary operators and that in the official notation we should write $\Ta_l(\Gamma,a)$. However, in the notation above we omit $\Gamma$ and write $\Ta_l(a)$.
...
Moreover, we have equations that the operator symbols commute with the cwf-operation of substitution in terms: \begin{eqnarray*}
