...
The extra structure for level-indexed scwfs is given by the following operator symbol $\leq$ for level equality types:
\footnote{equality reflection in ordinary type theory is
\begin{eqnarray*}
\Gamma : \ctx, A : \ty(\Gamma), a, a' : \tm(\Gamma, A),p :  \tm(\Gamma,\Id_A(a,a'))&\vdash& a = a' : \tm(\Gamma,A)
\end{eqnarray*}

Equality reflection for level equality will be defined later
\begin{eqnarray*}
n : \lctx, l, l' : \ltm_n, p : \ctm_n(\leq(l,l')) &\vdash& [l] = [l]' : \ltm_n/\psi
\end{eqnarray*}
}
\begin{eqnarray*}
n : \lctx, l, l' : \ltm_n &\vdash& \leq(l,l') : \cty_n
\end{eqnarray*}
It follows that a constraint "type" $c : \cty_n$ is a level equation 
...
Reasoning about level equality will be performed on the judgmental level. To this end we need to introduce a new sort for level terms with respect to constraints:
\begin{eqnarray*}
n : \lctx, \psi : \cctx_n &\vdash& \ltmq_n(\psi)
\end{eqnarray*}
This is to be understood as the quotient sort of $\ltm$ with respect to the equivalence relation generated by the constraints in $\psi$ and the laws for $\vee$ and $(-)^+$. We have a new operator symbol $[-]$ for equivalence class formation and an equation:
\begin{eqnarray*}
...
\end{eqnarray*}
Note that this is related to equality reflection: from a "propositional" level equality $\leq(l,l')$ we derive a judgmental level equality, and it is the latter that enables equational reasoning about $\vee$ and $(-)^+$ and $\psi$.
...
as above and get 
\[
p : \ctm_2(\psi,\leq(\beta\vee\alpha,\beta))\vdash 
[\beta\vee\alpha] = [\beta] : \ltmq_2(\psi).
\]
With this particular instantiation, the
...
\[
\vdash [\beta\vee\alpha] = [\beta] : \ltmq_2(\psi)
\]
that is, (MB: discuss) $\beta\vee\alpha$ and $\beta$ are equivalent
modulo $\psi$, in the empty context.

In much the same way, using the instantiation $l\equiv\alpha,~
l'\equiv\alpha\vee\beta$ and $\q[\p]$ instead of $\q$, we can get from
\[
p : \ctm_2(\psi,\leq(\alpha,\alpha\vee\beta))\vdash 
[\alpha]=[\alpha\vee\beta] : \ltmq_2(\psi).
\]
that $\alpha$ and $\alpha\vee\beta$ are equivalent modulo $\psi$, 
in the empty context.

Next we explain how we can get, e.g.,
$
\vdash [\alpha] = [\beta] : \ltmq_2(\psi).
$
This result cannot be obtained by simply using the transitivity of $=$,
we also have to show that
$
\vdash [\beta\vee\alpha]=[\alpha\vee\beta] : \ltmq_2(\psi).
$
The latter follows from 
$\vdash \beta\vee\alpha=\alpha\vee\beta : \ltm_2$ and congruence.
\end{example}
...
and so on, to get the equations reflected by these level equality types.
...
$\vdash [l]=[l'] : \ltmq_n(\psi)$ if and only if $l=l'$ can be inferred in 
lattice theory from the equalities reflected by the level
equality types occurring in $\psi$. 
\end{lemma}
