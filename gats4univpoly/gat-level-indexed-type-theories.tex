...
In this paper we carry out a case study for this approach. We present two gats, $\Sigmaext$ and $\Sigmaint$ and outline proofs that two of the theories $\TText$ and $\TTint$ in our paper {\em Type Theory with Explicit Universe Polymorphism} are initial in the respective categories $\CwF_\Sigmaext$ and $\CwF_\Sigmaint$. To provide full rigorous (even implemented) proofs of these initiality conjectures are outside the scope of the paper.
...
\item $\hom_n(\Delta,\Gamma)$ is the set of equivalence classes of raw context morphisms. 
