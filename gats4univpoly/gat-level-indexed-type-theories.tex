...
The aim of Voevodsky's {\em initiality conjecture project} \cite{voevodsky:initiality} is to provide a definition of a general class of dependent type theories and to develop generic metatheory for theories in this class. In particular, one would like a generic construction showing that theories in this class are initial in corresponding categories of models. Such proofs may seem straightforward superficially, but depend on subtle details in the formulation of grammar and inference rules. Hence, Voevodsky insisted on calling such theorems ``conjectures'' until proven rigorously and ideally implemented in a proof assistant.
...
Moreover, the gat of cwfs has operator symbols for the basic operations of cwfs (identity and composition of context morphisms, substitution in types and terms, empty context and context morphism, context extension and context morphism extension, projection morphism, last variable term): 
\begin{eqnarray*}
...
\end{eqnarray*}
where $\gamma : \Hom(\Delta,\Gamma)$ and $\gamma^+ = \langle \gamma \circ \p_{\Delta,\Ta_l(a[\gamma])}, \q_{\Delta,\Ta_l(a[\gamma])}\rangle : \Hom(\Delta.\Ta_l(a[\gamma]),\Gamma.\Ta_l(a))$.
...
\paragraph{Syntax and inference rules as an initial model.} Brunerie and de Boer \cite{Brunerie:initiality,deBoer:lic} implemented a proof that Martin-Löf type theory with an external tower of universes is an initial contextual category with appropriate extra structure for the type formers. Contextual categories are closely related to contextual cwfs \cite{castellan:lambek} and we expect that the implementation could be adapted accordingly, since initial cwfs are contextual. Several equivalences and embeddings between categorical notions of models including cwfs have been implemented in UniMath by Ahrens, Lumsdaine, and Voevodsky \cite{AhrensLV18}. On the syntactic side, our version of Martin-Löf type theory with an external tower of universes in Bezem et al \cite{BezemCDE22} is not presented in full detail as needed for formalization. There are also some differences, for example we use named variables rather than de Bruijn indices. A significant difference is that Brunerie and de Boer work with decorated terms. For example, application is decorated with type information. The insight that this is needed is due to Streicher \cite{streicher:thesis} who proved an initiality theorem for the Calculus of Constructions.

\paragraph{Internalization (* Remove? *).}
A simple way to internalize the externally indexed tower of universes would be to index each universe by internal natural numbers: 
...
Note that $l$ here depends on variables in the context $\Gamma$. Alternatively, one may want only to allow constant internal natural numbers:
$$ l : \tm(1,\N) \vdash \UU_l : \ty(\Gamma)$$

Similarly, Agda has a type $\Level$ of universe levels with a level 0 and closed under the next level operation $(-)^+$ and $\vee$. 

However, if universe levels form a type we can quantify over them, something we wish to avoid. This is the main motivation for designing a type theory with universe level judgments rather than a type of levels.

\section{Level-indexed type theory}

\paragraph{The ucwf of levels.} As already mentioned in the introduction, in Bezem et al \cite{BezemCDE22} we added special universe level {\em judgments}:
$$
l\ \level
\hspace{5em}
l = l'
$$
to the usual judgment forms of Martin-Löf type theory. Moreover, universe level variables as well as ordinary term variables can be declared in a context. As in Agda, we have a next level operation $(-)^+$ and an operation $\vee$ for join of levels. Levels form an upper semilattice with respect to $\vee$ and $(-)^+$ is an inflationary operation, see below.

However, as in our previous paper \cite{BezemCDE22}, we have no level 0 for the first universe. It follows that all universes are polymorphic.
...
%\end{itemize}
%}
Levels can be organized as a ucwf. We have the following sort symbols:
\begin{eqnarray*}
&\vdash& \lctx\\
...
The operator symbols for ucwfs are simplified versions of those for cwfs, where all dependence on types is removed:
...
\end{eqnarray*}
%\end{tiny}
\paragraph{Lawvere theories.} (* Remove *) Ucwfs are closely related to Lawvere theories. One can prove that the latter are equivalent to contextual ucwfs, that is, ucwfs where each context has a length \cite{CCD:lambek}.

\paragraph{The level-indexed cwf of small types.}
...
The gat for the level-indexd cwf of small types has the following sort symbols in addition to those of the gat of levels:
\begin{eqnarray*}
n : \lctx &\vdash& \ctx_n\\
...
Similarly, the operator symbols are the same as the operator symbols for cwfs (with extra structure for the small type formers), except that they are also indexed by $n : \lctx$. The equations are modified accordingly. See the appendix for the list of operator symbols and equations.
...
\end{eqnarray*}
Level substitution commutes with small type formers too, but we only show the case for $\Pi$-types.
...
\paragraph{Level-indexed universes.} We now add the operator symbols and equations for level-indexed universes.
...
$$
and the decoding equation for the $l$th universe in the $m$th ($l < m$)
\begin{eqnarray*}
\Ta_{m}((\UU^m_l)_\Gamma) &=& (\UU_l)_\Gamma
\end{eqnarray*}
is now relative to the context
...
\paragraph{Adding cumulativity.} Adding an operator symbol for cumulativity can also be obtained by internalizing the lifting operations in the external tower:
\begin{eqnarray*}
...
\paragraph{The initiality conjecture for level-indexed Martin-Löf type theory.} Can we adapt Brunerie and de Boer's  initiality proof for Martin-Löf type theory with an external tower of universes \cite{Brunerie:initiality,deBoer:lic} to a proof that Martin-Löf type theory with explicit universe polymorphism and level judgments (call it $\TTUP$) in the sense of our paper \cite{BezemCDE22} is an initial model of the gat in the present paper? Apart from the adaptations already discussed (cwfs versus contextual categories and decorations of terms on the syntactic side) we need to extend the proof to take into account the level judgments on the syntactic side and the level-indexing on the categorical side. 

We first explain how to construct a ucwf of levels from the judgments of $\TTUP$. 
\begin{itemize}
\item We use named level variables, and we define an element $n : \lctx$ as a list of distinct level variables. (Alternatively, we could use de Bruijn variables in our theory so that $n : \lctx$ is the number of variables.)
\item An element of $\ltm(n)$ is an equivalence class of level terms generated by $\vee$ and $(-)^+$ from level variables with respect to the equivalence relation generated by the equations for $\vee$ and $(-)^+$. Corresponding to this, in $\TTUP$  we have judgments
$$
\Gamma \vdash l\ \level
\hspace{5em}
\Gamma \vdash l = l'
$$
where $\Gamma$ is a context which may contain both level variables and term variables in arbitrary order. So $\ltm(n)$ would be the equivalence class $[ l ]$ such that $\Gamma \vdash  l\ \level$ where $n$ is the list of level variables in $\Gamma$ and $[ l ] = [ l' ]$ iff $\Gamma \vdash l = l'$.

\item An element of $\lhom(m,n)$ is a sequence of length $n$ of elements of $\ltm(m)$.
\item The ucwf-operations are defined in the obvious way. For example, level substitution $l[\sigma]$ is an implicit operation defined by induction on $l$. If $n = (\alpha_n, \ldots, \alpha_1)$ and $\s(n) =  (\alpha_n, \ldots, \alpha_1, \alpha_0)$ are lists of distinct level variables, then $\lp_n = (\alpha_n, \ldots, \alpha_1) : \lhom(\s(n),n)$ and $\lq_n = \alpha_0 : \ltm(\s(n))$ are the projections.
\end{itemize}
To prove that this forms an initial ucwf of levels we note that 
Brilakis \cite{Brilakis18} has implemented a proof in Agda of the equivalence between two initial ucwfs: one with explicit substitutions and one with implicit substitution and de Bruijn variables. Since the ucwf with explicit substitutions is constructed directly from the operator symbols of ucwfs, Brilakis' proof is essentially the same as proving the initiality of the ucwf with implicit substutions. To prove the initiality of the ucwf of levels obtained as above (but with de Bruijn variables), we only need to extend Brilakis' proof to account for the extra structure for $\vee$ and $(-)^+$.

The next step is to prove the initiality of the gat for the level-indexed cwf with small types. 
\begin{itemize}
\item We can always divide a context $\Gamma \vdash$ in $\TTUP$ into two parts: the list of level variables $n : \lctx$ and the list of typings of term variables $\Gamma'$. We can then define $\ctx_n$ as the set of contexts $\Gamma'$ including the term variables in $\Gamma \vdash$. We need to divide by equal contexts.
\item Moreover, we can define $\ty_n(\Gamma')$ as the set of $A$ such that $n, \Gamma' \vdash A$ is a type. We need to divide by equal types.
\item Similarly, $\tm_n(\Gamma',A)$ as the set of $a$ such that $n, \Gamma' \vdash a : A$. We need to divide by equal terms.
\item Finally, $\hom_n(\Delta,\Gamma)$ are lists of $\tm_n(\Delta,A)$.
\end{itemize}
To compare with Brunerie and de Boer's initiality proof we need to replace the external numbers by internal level indices. They also consider cumulative universes, that only need one level-index for $\Pi$ (and other type formers).
...
\paragraph{Level-indexed products of types.}
...
universal level quantification, abstraction, and application. The new corresponding operator symbols are
\begin{eqnarray*}
...
Future work to consider calculus with constraints.

Related work? Qiits? Qiits are inductively defined (initial) gats inside dependent type theory. We have the sequence: inductive type, inductive family, inductive-inductive type, quotient inductive-inductive type.
