...
T : \L^\op \to \CwF^\Pi
$$
The resulting generalized algebraic theory consists of following parts: \begin{itemize}
...
\Gamma : \ctx&\vdash&(\UU^m_l)_\Gamma: \tm(\Gamma,(\UU_{m})_\Gamma)
\end{eqnarray*}
In our old paper we had a similar rule for the internally indexed version, which used the definition of $l < m$ by the level equation $m = l^+ \vee m$. If we try to represent this rule by the following operator symbol
\begin{eqnarray*}
m = l^+ \vee m, \Gamma : \ctx&\vdash&(\UU^m_l)_\Gamma: \tm(\Gamma,(\UU_{m})_\Gamma)
\end{eqnarray*}
we leave the framework of generalized algebraic theories, since they do not allow equations in contexts. In the next section we will show how to extend our calculus with such equational constraints.
