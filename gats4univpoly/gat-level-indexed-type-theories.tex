...
Hence we have level equation types in $\lty_2$, such as
...
\]
that is, (MB: discuss) $\beta\vee\alpha$ and $\beta$ are equal
...
\[
p : \ctm_2(\psi,\leq(\alpha,\alpha\vee\beta))\vdash \alpha=\alpha\vee\beta.
\]
that $\alpha=\alpha\vee\beta$ in context $2,\psi$.
...
and so on, to get the equations reflected by these level equation types.
