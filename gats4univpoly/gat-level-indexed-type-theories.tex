...
We need to ensure that any universe $\UU_l$ has a code $\UU^m_l$ in any larger universe $\UU^l$ for $l < m$, where the latter is defined as $l^+ \vee m = m$. We would like to  type of the operator symbol for such codes should be something like
\begin{eqnarray*}
n : \lctx, l, m : \ltm(n), l < m, \Gamma : \ctx_n&\vdash&(\UU^m_l)_\Gamma: \tm_n(\Gamma,(\UU_{m})_\Gamma)
\end{eqnarray*}
However, equational constraints are not allowed in argument types in gats. Hence, we introduce a new sort symbol for level equality:
\begin{eqnarray*}
...
\end{eqnarray*}
Note that if we can derive $l = l'$ in the initial model, then we can also derive $\refl(l) :  \leq_n(l,l')$.
\footnote{Discuss relationship with equality types in ordinary type theory.}

We now have a proper typing of the operator symbol $\UU^m_l$:
\begin{eqnarray*}
n : \lctx, l, m : \ltm(n), p : l < m, \Gamma : \ctx_n&\vdash&(\UU^m_l)_\Gamma: \tm_n(\Gamma,(\UU_{m})_\Gamma)
\end{eqnarray*}
having suppressed the proof $p : l < m$ as an argument to this operator symbol.
...
%\Ta(\N^0_\Gamma) &=& \N_\Gamma\\
\Ta_{l \vee l'}(\Pi^{l,l'}(a,b)) &=& \Pi(\Ta_l(a),\Ta_{l'}(b))\\
%\Ta_{m}((\UU^m_l)_\Gamma) &=& (\UU_l)_\Gamma (* wrong *)\\
...
%\Ta^{l \vee -}_{l}((\UU^m)_\Gamma) &=& (\UU^{l\vee m})_\Gamma
\end{eqnarray*}
Equations for commutativity of operator symbols wrt substitution ($\gamma : \hom(\Delta,\Gamma)$):
\begin{eqnarray*}
\Ta^m_l(a) [ \gamma ] &=& \Ta^m_l(a[ \gamma ] )\\
...
\end{eqnarray*}
%There are many equations. First for each $l$, $\U_l$ satisfies all the laws of a single universe, including laws of ordinary substitution. Then we have 
...
%\footnote{Should we include $\frac{l < m\ \ \ l'<m}{l \vee l'  < m}$?}
$$
\frac{}{l < l^+}
\hspace{3em}
\frac{l < m}{l < m^+}
...
Proof $\Leftarrow$: If  $n$ is the length of the level context, then level expressions have normal forms 
$$
...
$$ 
provided $p_i < q_i$ for $1 \le i \le m \le m' \le n$. It follows that $<$ indeed can be inductively generated by the two rules above. Because all the $\alpha_j$ for $m+1 \le j \le n$ can be added by using the second rule repeatedly with $m' = \alpha_j^{+^{q_{j}}}$. Then use the monotonicity wrt $(-)^+$ to decrease all remaining $p_i$ and $q_i$. Then repeat the process. Monotonicity of $(-)^+$ follows by induction on the rules.
...
\begin{itemize}
\item If $l < m$ ($l^+ \vee m = m$) we can define $\UU^m_l$ by the process described above.
\end{itemize}
