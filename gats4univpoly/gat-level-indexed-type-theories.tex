...
{\em Type Theory with Explicit Universe Polymorphism}, so that models of these theories are models of the corresponding type theories. We first consider Martin-Löf type theory with an external tower of universes, the models of which are categories with families with extra structure for the type formers including extra structure for the universes. We then consider our proposal for a type theory with explicit universe polynorphsm, where we have universe level judgments and an internally indexed tower of universes. The models of this theory are level-indexed categories with families with extra structure. Finally,. we extend the theory with universe level constraints. The models of these are doubly indexed categories with families with extra structure.
\end{abstract}
