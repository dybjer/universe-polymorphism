...
The latter equation for equality reflection deserves exemplification.
\begin{example}\label{exa:cctx-entails-lattice-eqs}
In level context $2$ we can use two level variables which we name $\alpha$ and 
$\beta$ for readability. In this context, these variables themselves
are level terms, and so are, e.g., $\alpha\vee\beta$ and $\beta\vee\alpha$.
Hence we have in this same context $2$ level equations, such as
...
which we name $\psi$ for short.
We can then instantiate the equation for equality reflection
...
and $\q[\p]$ instead of $\q$ we can get
\[
p : \ctm_2(\psi,\leq(\alpha\vee\beta,\alpha))\vdash \alpha\vee\beta=\alpha.
\]
and hence that $\beta\vee\alpha$ and $\beta$ are equal
in the context $2,\psi$.

Now, by reasoning about level equations, using commutativity and congruence,
we can also obtain, e.g., that $\alpha$ and $\beta$ are equal
in the context $2,\psi$.
\end{example}
