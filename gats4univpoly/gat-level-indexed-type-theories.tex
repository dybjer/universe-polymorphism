...
Sort symbols will be interpreted as types in the term model cwf. We have $\ctx \in \Ty(1), \sub \in \Ty(1.\ctx.\ctx[\p])$, etc. Hence, in the term model, $\sub(\Delta,\Gamma)$ is shorthand for $\sub[\tuple{\Delta,\Gamma}]$, where $\tuple{a} = \tuple{\tuple{}, a}$ and $\tuple{a,b} = \tuple{\tuple{a}, b}$.
