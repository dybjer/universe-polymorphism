...
Moreover, the gat of cwfs has 11 operator symbols for the basic operations of cwfs (identity and composition of context morphisms, substitution in types and terms, empty context, context extension, terminal (empty) context, terminal context morphism, context morphism extension, projection morphism, last variable term). To illustrate the notation we show the typings of identity and composition of context morphisms:
\begin{eqnarray*}
...
As a first step, we shall show the operator symbols and equations for an external tower of universes $\UU_l$, where $l \in \N$ is an external natural number. %Note that this is an infinitary theory.

We assume that we already have defined the generalized algebraic theory of cwfs with $\Pi$-types, see Bezem, Coquand, Dybjer, and Escard\'o \cite{bezem:hofmann}. The reader may also consult the appendix, where an indexed version of this theory is presented: each sort symbol and operator symbol has an additional first argument $n : \lctx$. 

We should of course include operator symbols corresponding to the formation, introduction, and elimination rules, and equations for the other standard type formers of Martin-Löf type theory as well, and let $\UU_l$ be closed under those too. However, throughout the paper we will only show closure under $\Pi$-types and leave the extension to the other standard type formers to the reader.
