...
\paragraph{A construction based on implicit substitutions.} 
We shall now outline an alternative construction of an initial object in $\CwF_{\Sigmaext}$ from $\TText$ -- the type theory with an external tower of universes presented in Bezem et al \cite{BezemCDE22}. This theory is a more standard version of Martin-Löf type theory with implicit substitutions, where substitution is an operation defined by induction on types and terms. As we state in that paper, we first need to modify our syntax and work with decorated terms; for example, we need to decorate application with type information. 
