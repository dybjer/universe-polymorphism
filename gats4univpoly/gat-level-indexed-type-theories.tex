...
The final part of the proof is about the arrow part of the level-indexed cwf with small types and level-indexed universes. In $\mathbf{TTUP}$ \MB{explain} \footnote{PD: Should we introduce abbreviations for the gats and the type theories? For example, 
...
As such they are encodings of the syntactic theories $\TText$ and $\TTint$ rather than the syntactic theories themselves. 
...
Another topic for further research is how to capture the theory of explicit universe polymorphism with equational constraints \cite{BezemCDE22} as a gat. %We plan to show how level equality sorts $\leq_n(l,l')$.
