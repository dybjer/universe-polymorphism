...
A = A' : \ty(\Gamma)
$$
and we do not need a new sort symbol corresponding to type equality judgments. However, this principle breaks down when formalizing the principle that any universe has a code in any larger universe. This is because larger than $l < l'$ is defined as the level equality $l^+ \vee l' = l'$ and level equalities are not allowed in contexts in gats. Hence we need to add a sort $\leq_n(l,l')$ for level equality.

In a forthcoming article we will explain how to capture the theory of explicit universe polymorphism with equational constraints \cite{BezemCDE22} as a gat. This also makes essential use of level equality sorts $\leq_n(l,l')$.
...
%$$}
\appendix
\section{Operator symbols and equations for tComplete gat for the level-indexed cwf of small types}

We already listed the sort symbols. 
