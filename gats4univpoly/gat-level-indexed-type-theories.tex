...
In Section 6 we discuss related and further research. There is also an appendix with operator symbols and equations for level-indexed type theory that were not included in Section 3.
...
\paragraph{Equations.}
Finally, the gat of cwfs has 13 equations. We illustrate the notation by showing one of the laws for identity morphisms.\begin{eqnarray*}
...
\paragraph{A generic construction based on explicit substitutions.} In our paper {\em On generalized algebraic theories and categories with families} \cite{bezem:hofmann} we define the notion of a correct {\em presentation} (a finite list of sort symbols, operator symbols, and equations) $\Sigma$ of a gat and the associated category of models $\CwF_\Sigma$ of cwfs with a $\Sigma$-structure. We then construct an initial object $\T_\Sigma$ in $\CwF_\Sigma$ as a type theory defined in terms of a grammar and inference rules. This type theory is a calculus of explicit substitutions based on untyped cwf-combinators and untyped versions of the operator symbols. 

In the current section we have presented the {\em infinitary} gat of cwfs with extra structure for the small type formers and a tower of universes, while the above construction of an initial model applies only to {\em finitary} gats. However, as explained in Section 5.7 of Bezem et al  \cite{bezem:hofmann}, we can generalize our construction to some non-finitely presented gats. If we have an increasing sequence of signatures $\Sigma_n$ we can build the initial model $\T_\Sigma$ of their union $\Sigma$ in stages. In this way we can build an initial model by letting $\Sigma_n$ be the gat of cwfs with extra structure for the small type formers and a truncated tower of $n$ universes. 

We remark that when $\Sigma$ presents cwfs (with extra structure), $\CwF_\Sigma$ is the category of cwfs with an {\em internal} cwf (with extra structure) and $\T_\Sigma$ is a cwf with an internal cwf (with extra structure). Since any gat is built on top of a basic framework of dependent types, its models are cwfs with extra structure. When we consider the gat of cwfs the extra structure is that of an internal cwf.
...
What is the relationship with Brunerie and de Boer's \cite{Brunerie:initiality,deBoer:lic} construction of an initial contextual category with extra structure for an external tower of universes? Their term model is also based on a more standard version of Martin-Löf type theory with implicit substitutions, but with decorated terms. On the semantic side there are several differences, but we consider those minor. First, their notion of model is that of a contextual category (with extra structure) while ours is a cwf (with extra structure). These are closely related notions, taking into account that initial cwfs are contextual in the sense that each context has a length \cite{ClairambaultD11,ClairambaultD14}. Brunerie and de Boer's structure for universes is also different: they just have the rule $\UU_l : \UU_{l+1}$ and only consider the non-cumulative case, while we have $\UU_l^m$ for $\UU_l$ in any larger universe $\UU_m$ and consider both the non-cumulative and cumulative cases.  
