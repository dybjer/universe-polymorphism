...
{\em Type Theory with Explicit Universe Polymorphism}, so that models of these theories are models of the corresponding type theories. We first consider Martin-Löf type theory with an external tower of universes, the models of which are categories with families with extra structure for the type formers including the universes. We consider type theory with universe level judgments, internal universe level variables, and an internally indexed tower of universes. The models of this theory are level-indexed categories with families with extra structure. Finally,. we present extend the theory with universe level constraints. The models of these are doubly indexed categories with families with extra structure.
\end{abstract}
