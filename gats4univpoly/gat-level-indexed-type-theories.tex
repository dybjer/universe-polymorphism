...
Sort symbols will be interpreted as types in the term model cwf \cite{bezem:hofmann}. This is a calculus of explicit substitutions based on cwf-combinators. We have $\ctx \in \Ty(1), \sub \in \Ty(1.\ctx.\ctx[\p])$, etc., and use $\sub(\Delta,\Gamma)$ as shorthand for $\sub[\tuple{\q[\p],\q}]$, where $\tuple{a,b} = \tuple{\tuple{a}, b}$ and $\tuple{a} = \tuple{\tuple{}, a}$.
...
Operator symbols will be interpreted as terms in the term model cwf. For example, $\id \in \Tm(1.\ctx,\ty)$ and $\id_\Gamma$ is shorthand for $\id[\tuple{\Gamma}]$. 
