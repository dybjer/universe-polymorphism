...
We also presented an extension with equational constraints between universe levels, building on a proposal by Voevodsky \cite{VV}. To this end we added yet another judgment form
$$
\psi\ valid
$$
...
\paragraph{The object part of the level-indexed cwf of types.} This is like the cwf with extra structure for ordinary type theory, but now all the structure depends on a level context $n$.

\paragraph{The arrow part of the level-indexed cwf of types.} 
\begin{itemize}
\item
...
The extra structure for level-indexed scwfs is given by the following operator symbol $\leq$ for {\em level equality} as types in the level-indexed scwf of constraints:
\footnote{equality reflection in ordinary type theory is the following equation in generalized algebraic theories:
\begin{eqnarray*}
...
(Talk about initial model and the notion of derivation.)
In level context $2:\lctx$ (where we define $2 = \s(\s(0)) : \lctx$ and remark that level contexts need not be natural numbers) we can use two level variables which we 
name $\alpha$ and $\beta$ for readability. (Officially, $\alpha = \q[\p] : \ltm_2$ and $\beta = \q  : \ltm_2$ in the ucwf of levels.)
...
%\[
%\vdash [\beta\vee\alpha] = [\beta] : \ltmq_2(\psiab)
%\]
that is, (MB: discuss) $\beta\vee\alpha$ and $\beta$ are equivalent
modulo $\psiab$, in the empty context.)

In much the same way, using the instantiation $l\equiv\alpha,~
l'\equiv\alpha\vee\beta$ and $\q[\p]$ instead of $\q$, we get
\[
%p : \ctm_2(\psiab,\leq(\alpha,\alpha\vee\beta))
\vdash [\alpha]=[\alpha\vee\beta] : \ltmq_2(\psiab).
\]
...
$
\vdash [\alpha] = [\beta] : \ltmq_2(\psiab).
$
This result cannot be obtained by simply using the transitivity of $=$,
we also have to show that
$
\vdash [\beta\vee\alpha]=[\alpha\vee\beta] : \ltmq_2(\psiab).
$
The latter follows from 
$\vdash \beta\vee\alpha=\alpha\vee\beta : \ltm_2$ and the fact that operator symbols preserve equality in gats.
\end{example}
...
$\psi:\cctx_n$ a constraint context, all indexed by $n$. If we have closed $\vdash n : \lctx, \vdash \psi : \cctx_n,  \vdash l, l' : \ltm_n$ (as in initial model) then we have:
\begin{eqnarray*}
&\vdash& [l]_\psi  = [l']_\psi  : \ltmq_n(\psi)
\end{eqnarray*}
...
%Do we have the following stronger
%\begin{eqnarray*}
%n : \lctx, \psi : \cctx_n,  l, l' : \ltm_n &\vdash& [l]_\psi  = [l']_\psi  : \ltmq_n(\psi)
%\end{eqnarray*}
%f and only if $l=l'$ can be inferred by equational reasoning from the laws for $\vee$ and $(-)^+$ and the equations reflected by the level
%equalities occurring in $\psi$?
