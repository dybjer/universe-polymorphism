...
We have a new operator symbol $[-]$ for equivalence class formation with its equation:
\begin{eqnarray*}
...
name $\alpha$ and $\beta$ for readability. (Officially $\alpha = \q[\p] : \ltm(2)$ and $\beta = \q  : \ltm(2)$ in the ucwf of levels.)
...
so that we can form a constraint contexts, such as 
...
with $n\equiv 2$, $l\equiv \beta\vee\alpha, l'\equiv \beta, \psi:= \psiab$, and $p := \q: \ctm_2(\psiab,\leq(\beta\vee\alpha,\beta))$\footnote{check signs for instatiation and definition}
as above and get 
\[
...
$\psi:\cctx_n$ a constraint context, all indexed by $n$. If we have concrete $\vdash n : \lctx, \vdash \psi : \cctx_n,  \vdash l, l' : \ltm_n$ then we have:
\begin{eqnarray*}
