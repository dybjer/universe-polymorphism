...
\and Thierry Coquand\\University of Gothenburg\\
\texttt{Thierry.Coquand@cse.gu.se}\\ 
\and Peter Dybjer\\Chalmers University of Technology\\
...
An implicit form of universe polymorphism was introduced by Huet \cite{Huet87} and is an essential feature of the proof assistant Rocq (Coq) \cite{rocq:general}. Alternatively, Agda \cite{agda-wiki} and Lean \cite{moura:lean} employ versions of universe polymorphism, where universe levels are explicitly declared.
...
%\todo[inline]{After the reorganization, we can end the introduction here.}
...
%\todo[inline]{Add something about the general correspondence between gats and type theory:
%sort symbol = judgment form, operator symbol for formation, introduction, and elimination rules, equation for equality rules.}
%
...
\end{eqnarray*}
and we refer to Bezem et al \cite{bezem:hofmann} for the operator symbols for abstraction and application, equations for the $\beta$ and $\eta$ rule, and equations expressing that $\Pi$, abstraction, and application commute with term substitution. We also refer to the appendix for the level-indexed version.
...
&\vdash&
 ...
 \end{eqnarray*}
We do not display the equations for the cumulative case here and refer to the analogous equations level-indexed universes in \ref{cumulativity}.
...
%\todo[inline]{PD: Thierry and I discussed it and it seems important. The verification doesn't seem difficult. Nevertheless, it's too late to include it. Should we skip this whole comment?}
%
...
%In contrast to the construction of an initial object in $\CwF_{\Sigmaext}$ the category of cwfs with an internal cwf (with extra structure) we now construct an initial object in $\CwFext$, the category of cwfs (with extra structure).
%\todo[inline]{We not only need to contrast $\CwFext$ and $\CwF_{\Sigmaext}$, but explain what $\CwFext$ is.}
...
However, as we shall see in the next subsection, when typing the codes $\UU_l^m$ for universes $\UU_l$ in $\UU_m$ we need to express the constraint that $l < m$ which is defined as $\leq_n(l^+ \vee m, m)$. However, level equalities are not allowed in gats. Therefore, we add a new sort symbol for level equality:
\begin{eqnarray*}
...
\end{eqnarray*}
Note that if $l =  l' : \ltm(n)$ in the term model, that is, if $l = l'$ can be derived by equational reasoning from the laws for $\vee$ and $(-)^+$, then $\refl(l) :  \leq_n(l,l')$
%\begin{eqnarray*}
...
\todo[inline]{TC: Remark on finitely presented semilattices and constraints?}
...
%\todo{
%We should give more information here. The $p$ below is unexplained. PD: I have uncommented the type information. Although it is useful to check this, we can leave this to the reader.}
\begin{eqnarray*}
\Gamma[\lid_n] &=& \Gamma\\
...
\begin{eqnarray*}
\Ta^{m \vee m'}_{l \vee l'}(\Pi^{l,l'}(a,b)) = \Pi^{m,m'}(\Ta^m_l(a),\Ta^{m'}_{l'}(b)) &:& \tm_n(\Gamma,(\UU_{m \vee m'})_\Gamma)
\end{eqnarray*}
where $n: \lctx,l,l',m,m' : \ltm(n), p: l < m, p': l' < m', \Gamma : \ctx_n, a : \tm_n(\Gamma,(\UU_l)_\Gamma), b : \tm_n(\Gamma.\Ta_l(a),(\UU_{l'})_\Gamma)$.
...
\paragraph{A construction based on implicit substitutions (initiality conjecture).} We shall outline how to construct an initial object $\Tint = (\Lctx,\Lsub,\Ltm,\Ctx,\Sub,\Ty,\Tm)$ in $\CwFint$ based on $\TTint$, the decorated version of our type theory with explicit universe polymorphism \cite{BezemCDE22}. 
...
%The details are similar to the construction of a model based on $\TText$.
%\todo{Be systematic with using superscripts and subscripts!}
