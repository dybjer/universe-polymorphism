...
\begin{eqnarray*}
\Ta^m_{l \vee l'}(\Pi^{l,l'}(a,b)) &=& \Pi(\Ta^m_l(a),\Ta^m_{l'}(b))\\
...
\end{eqnarray*}
We remark that in the presence of cumulativity it suffices with one superscript on codes for $\Pi$, that is, we have $\Pi^l$ rather than $\Pi^{l,l'}$.
...
$$l : \Gamma : \ctx, l : \tm(\Gamma,\N) \vdash U_l : \ty(\Gamma)$$. 
However, this would mean that we could build large types such as $\Pi l : \N.\U_l$ and $\Pi l : \N.\U_l$ which increase the proof theoretic strength of type theory. 

In Agda, there is a type $\Level$ which has a level 0, and is closed under the next level operation $(-)^+$ and $\vee$. Also here we can build $\Pi l : \Level.\U_l$ and $\Pi l : \Level.\U_l$
...
The rule of universe formation states that for each level term $l$ in $n$ variables, there is a type $\UU_l$ with decoding map $\Ta_l$. Then there is an  Then there is an introduction rule expressing that every universe is in the next. 
...
\end{eqnarray*}
where $l < m$ is defined as $l^+ \vee m = m$, does not work since equations are not allowed in argument types in gats. This issue can be solved by introducing a new sort symbol for level equality:
\begin{eqnarray*}
