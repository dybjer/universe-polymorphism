...
A significant difference is that Brunerie and de Boer work with decorated terms following Streicher \cite{streicher:thesis}. For example, application is decorated with type information. 
...
However, if universe levels form a type we can quantify over them, something we wish to avoid. This is an important reason for designing a type theory with universe level judgments rather than a type of levels.
...
\end{eqnarray*}
%\end{tiny}
\paragraph{Remark.} We remark that uwfs are closely related to Lawvere theories, but closer to the usual syntax based on $n$-place functions. One can prove that the latter are equivalent to contextual ucwfs, that is, ucwfs where each context has a length \cite{castellan:lambek}.
...
The gat for the level-indexed cwf of small types has the following sort symbols in addition to those of the gat of levels:
\begin{eqnarray*}
n : \lctx &\vdash& \ctx_n\\
...
\paragraph{Level-indexed universes.} We finally add the operator symbols and equations for level-indexed universes.
...
\paragraph{Adding cumulativity.} Adding an operator symbol for cumulativity is obtained by internalizing the lifting operations in the external tower:
\begin{eqnarray*}
...
\end{itemize}
We then replace the external tower of universes in Brunerie and de Boer's initiality proof we need by internally level-indexed ones. We we also need to take into account that Brunerie and de Boer consider non-cumulative universes with singly rather than doubly indexed codes for $\Pi$ and the other small type formers.
...
\paragraph{Remark.} In the introduction we mentioned that untyped predicate logic can be captured by ucwf-indexed scwfs. (This gives a proof-relevant logic, just like Lawvere's hyperdoctrines \cite{lawvere:hyperdoctrine}.) We note that the operator symbols and equations for universal quantification in untyped predicate logic are the same as those for universal level quantification above, except the difference between scwfs and cwfs: types (propositions) in predicate logic do not depend on proofs.
