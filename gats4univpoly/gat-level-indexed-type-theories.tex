...
However, then we can quantify over levels and form $\Pi l : \N.\UU_l$ % ($(\Pi(\N,\(\UU_l)_\Gamma)$ 
and $\Sigma l : \N.\UU_l$, something we wish to avoid. 
...
\subsection{Level-indexed universes.} We finally add the operator symbols and equations for level-indexed universes.
...
\subsection{Adding cumulativity.} Adding an operator symbol for cumulativity is obtained by internalizing the lifting operations in the external tower:
\begin{eqnarray*}
...
The next step is construct the initial of the gat for level-indexed cwfs with small types. 
\begin{itemize}
...
\item The level equality sort $p : \leq_n(l,l')$ is interpreted in the same way as $l = l' : \ltm(n)$ in the initial model, that is, by the judgment $n \vdash l = l'$.
\end{itemize}
