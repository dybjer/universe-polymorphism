...
\end{eqnarray*}
Note that if we can derive $l = l' : \ltm(n)$ in the initial model, then we can also derive $\refl(l) :  \leq_n(l,l')$.
...
\end{eqnarray*}
where $l < m$ is defined as $\leq_n(l^+ \vee m, m)$. In the sequel we will suppress the proof $p : l < m$ as an argument to this operator symbol.
