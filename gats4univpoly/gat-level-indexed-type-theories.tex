...
\paragraph{Syntax and inference rules as an initial model.} We would like to prove that the syntax and inference rules of for Martin-Löf type theory with an external tower of universes in Bezem et al \cite{BezemCDE22} can be organized as an initial model of our gat. A closely related proof has been implemented in Agda by de Boer and Brunerie \cite{Brunerie:initiality,deBoer:lic}. They prove that a slightly different version of Martin-Löf type theory with an external tower of universes is an initial contextual category with appropriate extra structure. Contextual categories are closely related to contextual cwfs \cite{castellan:lambek} and we expect that the implementation can be adapted accordingly. We also remark that several equivalences and embeddings between categorical notions of models including cwfs have been implemented in UniMath by Ahrens, Lumsdaine, and Voevodsky \cite{AhrensLV18}. 
