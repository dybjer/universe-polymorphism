...
\paragraph{The formal system as a cwf with extra structure.} A proof that a version of Martin-Löf type theory with a tower of universes is an initial contextual category has been implemented in Agda by de Boer and Brunerie \cite{deBoer:lic,Brunerie:talk}. This is for a version of type theory with de Bruijn variable and implicit substitutions. The detailed definition of the syntax and inference rules gives an opportunity to different choices. On the semantic side, cwfs are not exactly like contextual categories, but contextual cwfs are closely related, and should be equivalent.
