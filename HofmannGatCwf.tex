...
We make a few remarks. $\Ctx = \Obj$ is the constant sort of objects of the category of contexts in a cwf. $\Hom$ is a binary sort symbol for the context morphisms in the cwf. These typing of the two arguments are represented by the context $\Delta, \Gamma : \Ctx$. In our type theory we have an operation which maps a type $\Gamma \vdash A$ and a substitution $\Delta \vdash \gamma : \Gamma$ to $\Delta \vdash A[\gamma]$ (defined below). Hence when we apply $\Hom$ to two concrete arguments $\Delta,\Gamma$ we get the result $\Hom(\Delta,\Gamma)$ by using this substitution operation. (More to say ...)
...
\section{Signatures and models of generalized algebraic theories}
 ...
 We shall now define how to build a valid gat signature $\Sigma$ and what it means for a cwf to support it. This definition relies on the construction of initial cwfs $\T_\Sigma$ supporting $\Sigma$, but will postpone the construction of these until the next section. In this way we separate the abstract definition from the concrete syntactic details employed for building $\T_\Sigma$. 
...
The interpretation morphism $\inte{-}'$ is defined as $\inte{-}$ but on the new equivalence classes
\footnote{Martin: clarify!}. It immediately follows that it is a morphism in $\Cwf_{\Sigma'}$ and that it is unique.\end{description}

\section{Examples of generalized algebraic theories}

\subsection{Monoids} A sort symbol $M$. operator symbols $\id$ for identity and $\circ$ for composition, and associativity and identity laws as equations. This is an ordinary algebraic theory.
