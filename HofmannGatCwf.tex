...
An example of a cwf with an internal cwf is provided by the cwf $\Set$ of small sets with an internal category of "very small sets". We can make this precise if we assume that we work in set theory with two Grothendieck universes $\V_0$ and $\V_1$ with $\V_0 \in \V_1$. We call the members of $\U_1$ "small sets" and the members of $\U_0$ "very small sets". The category of contexts the cwf $\Set$ is the usual category of small sets, and the family valued functor maps a small set to a family of small sets. To get an internal cwf, we interpret the sort of objects as the small set $\V_0$, the sort of morphisms as a family of small sets (indexed by very small sets), and the sort of types (indexed by very small sets) as a family of small sets, etc.

Further examples of internal cwfs inside $\Set$ are provided by freely generated cwfs, and cwfs supporting signatures of generalized algebraic theories, as constructed in the previous section.
