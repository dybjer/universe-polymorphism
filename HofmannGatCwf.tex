...
Generalized algebraic theories (gats) were originally introduced by Cartmell in his PhD thesis \cite{cartmell:phd} as a dependently typed generalization of many sorted algebraic theories. Each gat is specified by a signature with (possibly infinite) sets of sort symbols, operator symbols, and equations. Cartmell's definition is in the spirit of traditional syntactic systems for dependent type theory. He also defines a notion of model whereby sort symbols are interpreted as families of sets.

Voevodsky's initiality conjecture \cite{voevodsky:initiality} aims to characterize a whole class of "type theories" abstractly as initial objects in suitable classes of models and to prove that these initial objects exist. For example, we can define Martin-Löf type theory as the initial object in the category $\Cwf^{\Pi,\Sigma,\Id}$ of categories with families (cwfs) with extra structure for interpreting type formers such as $\Pi, \Sigma$ and identity types $\Id$. An explicit construction of this initial object has been constructed by Castellan, Clairambault, and Dybjer \cite{castellan:warsaw,castellan:lmcs}. 

We shall here define a finitary version of Cartmell's notion. Our definition is based on the notion of category with families (cwf) \cite{dybjer:torino}. We simultaneously define what it means to be a valid signature $\Sigma$ for a gat and the category $\Cwf_\Sigma$ of cwfs supporting $\Sigma$. This definition refers to the construction of initial objects in $\Cwf_\Sigma$ but is independent of the details of this construction. We show how to extend the construction of the initial object of $\Cwf$ to an initial object of $\Cwf_\Sigma$.
\footnote{
...
\begin{itemize}
\item The initial cwf $\T_\emptyset$ (as constructed above) has only one object (context) 1, one equivalence class of morphisms $\id_1$, and no types and terms. Hence we can add a new constant sort $\Obj$ of (internal) objects with context $1$ to the signature. 
