...
\item Operations ${\cext\,},~\p,~\q$ and $\tuple{\_,\_}$ 
...
If $T(\Gamma) = (U_x)_{x\in X}$, then we write $\Ty(\Gamma)$ for the set $X$.
...
${\cext\,},~\p,~\q,~\tuple{\_,\_}$.
Given a $\Gamma \in \C$ and type $A \in \Ty(\Gamma)$,
we have
...
\] 
We call $\Gamma \cext A$ an \emph{extended} context 
and $\p$ the \emph{first projection}. 
If, furthermore, $\gamma : \Delta \to \Gamma$ 
and $a\in \Tm(\Delta, A[\gamma])$, then 
\[
\tuple{\gamma, a} : \Delta \to \Gamma \cext A.
\]
We call $\tuple{\gamma, a}$ an \emph{extended} substitution.

The operations  ${\cext\,},~\p,~\q,~\tuple{\_,\_}$
...
The gat of categories (with $\Ctx = \Obj$)
...
\subsection{The extension with a family valued functor}

\subsection{The extension with a terminal object}

\subsection{The extension with context comprehension}
...
\\&&\Gamma \vdash a : A
\end{eqnarray*}
are then defined as the reflexive instances of the partial equivalence relations. The initial cwf $\T$ is then constructed from the equivalence classes of derivable judgments. For example, the contexts in $\T$ are equivalence classes $[\Gamma]$, such that $\Gamma \vdash$. See Castellan, Clairambault, and Dybjer \cite{castellan:tlca2015,castellan:lmcs} for details.
