...
Assume now for the induction step that we have specified the grammar and the inference rules for $\Sigma$.
Let $\Sigma'$ be $\Sigma$ extended by a new sort symbol, a new operator symbol, or a new equation. We shall now explain how to build the extended grammar and the extended set of inference rules for $\Sigma'$.
\begin{description}
\item[Adding a new sort symbol $F$ in $\Gamma$] 
We show how the four parts change:
\begin{itemize}
\item
...
\begin{mathpar}
  ...
  \end{mathpar}
to the inference rules for $\Sigma$. (Note that the context $\Gamma$ represents the types of the arguments of $F$.)
\item
...
\item[Adding a new operator symbol $f$ of type $A$ in context $\Gamma$] 
We modify the four parts as follows:
\begin{itemize}
\item
...
\begin{mathpar}
  ...
  \end{mathpar}
to the inference rules for $\Sigma$. (Note that the context $\Gamma$ represents the types of the arguments of $f$ and $A$ is the type of the result.)
\item
We define $f_{\T_{\Sigma'}} = [f]$. It follows that $ \T_{\Sigma'}$ supports $\Sigma'$. 
\item
We extend the definition of the interpretation morphism $\inte{-}$  to an interpretation morphism $\inte{-}' : \T_{\Sigma'} \to \C$ by 
