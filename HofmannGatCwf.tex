...
to a purely syntactic definition of an {\em initial object} $\T_{\Sigma}$ in $\Cwf_{\Sigma}$ (for all valid signatures $\Sigma$) in terms of grammars and inference rules. A context will be an equivalence class $[ \Gamma ]$ of raw contexts, and similarly for substitutions, types, and terms. To give a uniform
...
To construction of $\T_\Sigma$ is the topic of the next section.
...
\subsection{Monoids} The one-sorted algebraic theory of monoids has two operator symbols,
...
We shall now show in some detail how to build a valid signature for monoids 
...
As a rule of the thumb: these inference rules closely follow Definition~\ref{def:Cwfobj}.)
