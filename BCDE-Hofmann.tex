...
much weaker. We will however need to make certain size distinctions. For the general theory we need to distinguish between small and large sets, and for this purpose we assume that our set theory comes with a Grothendieck universes $\V$ of {\em small sets}. Furthermore, in Section \ref{sec:examples} we provide the example of the small cwf $\Set'$ of tiny sets with an internal category of very tiny sets. This relies on our set theory having two more Grothendieck universes $\V'$ of {\em tiny sets} and $\V''$ of {\em very tiny sets}, where $\V''  \in \V' \in \V$.
...
If $\Sigma$ is the presentation of the generalized algebraic theory of cwfs, then $\Cwf_\Sigma$ is the category of small cwfs with an internal cwf. An example is the small cwf $\Set'$ of tiny cwfs with an internal cwf of very tiny sets. A tiny cwf 
%is a cwf in the ordinary sense (see Definition~\ref{def:Cwfobj}) except that it 
has tiny sets of objects, morphisms, types, and terms. As mentioned in the introduction, we assume two more Grothendieck universes $\V'$ of {\em tiny sets} and $\V''$ of {\em very tiny sets}, where $\V''  \in \V' \in \V$, and $\V$ is the set of small sets. %The contexts and types of $\Set'$ are tiny sets. 
To get an internal cwf, we interpret its sort of objects $\ctx$ as the tiny set $\V''$ of very tiny sets, and the sorts of types $\ty(\Gamma)$ also as $\V''$.
