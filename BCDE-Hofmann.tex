...
%We shall here define a new finitely presented notion of gat and simultaneously a general categorical notion of model. To this end we define what it means to be a \FYI{presentation} $\Sigma$ of a gat and the associated category $\Cwf_\Sigma$ with extra structure for $\Sigma$. Our definition refers to {\em uniform families} of contexts, types, and terms in $\Cwf_\Sigma$, a purely semantic notion. Afterwards, we construct initial objects $\T_\Sigma \in \Cwf_\Sigma$ by extending Castellan, Clairambault, and Dybjer's  construction of an initial object in the category $\Cwf$ of cwfs \cite{castellan:tlca2015,castellan:lmcs}.
...
\begin{remark}
Note the important difference between the internal notion of cwf defined here and the external notion of cwf defined in Section \ref{sec:def_cwf}. 
\end{remark}
...
\begin{remark}
Cartmell \cite{cartmell:apal} defines a gat for $\Sigma$-types as follows. If we start with any gat with a constant sort symbol $A$ and  a sort symbol $B$ in context $x:A$, then we can extend it with a new constant sort symbol $\Sigma B$ and operator symbols and equations for the two projections and the pairing. In a similar way we could extend any such gat with a new constant sort symbol $\Pi B$ and operator symbols for $\lambda$ and $\app$ and the equations for $\beta$ and $\eta$. The reader should be aware of the difference between the resulting gats and our gat for cwfs with $\Pi$-types.
